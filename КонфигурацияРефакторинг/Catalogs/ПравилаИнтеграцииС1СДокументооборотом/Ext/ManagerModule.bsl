...
Функция ПолучитьРеквизитыОбъектаДО(ТипОбъектаДО, Параметры = Неопределено, НастройкиВида = Неопределено,
	...
	Если ТипОбъектаДО = "DMIncomingDocument" Тогда
		...
		// Проекты.
		Если ПолучитьВсе Или (ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.3.2.3.CORP")
	...
	ИначеЕсли ТипОбъектаДО = "DMInternalDocument" Тогда
		...
		Если НастройкиВида <> Неопределено
				И ИнтеграцияС1СДокументооборот.СвойствоСуществует(НастройкиВида, "isAPaymentRequest")
		...
		Если НастройкиВида <> Неопределено
				И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НастройкиВида, "partiesEnabled")
		...
		Если ПолучитьВсе Или НастройкиВида = Неопределено Или НастройкиВида.sumEnabled = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "sum", НСтр("ru = 'Сумма'"), "Число");
			Если ПолучитьВсе
					Или ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.8.1.CORP") Тогда
		...
		Если ПолучитьВсе Или (ВестиУчетСторон И Не ЯвляетсяЗаявкойНаОплату)
			...
			ДобавитьРеквизит(
				...
				ТерминКонтрагент,
				"DMCorrespondent",
				?(ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.28.12.CORP"), Ложь, Истина));
			ДобавитьРеквизит(Реквизиты, "contactPerson", НСтр("ru = 'Контактное лицо'"), "DMContactPerson");
			
		КонецЕсли;
		
		Если ПолучитьВсе
				Или (ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.28.12.CORP")
		...
		Если ПолучитьВсе Или (НастройкиВида <> Неопределено
				И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НастройкиВида, "cashFlowDetailsEnabled")
		...
		Если ПолучитьВсе Или (НастройкиВида <> Неопределено
				И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НастройкиВида, "productRowsEnabled")
	...
	Если ТипОбъектаДО = "DMIncomingDocument"
		...
		Если ПолучитьВсе Или (ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1.CORP")
...
Функция СоответствиеСвойствXDTOиРеквизитовФормыОбъектаДО(ТипОбъекта) Экспорт
	...
	ИначеЕсли ТипОбъекта = "DMInternalDocument" Тогда
		...
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.28.12.CORP") Тогда
...
Функция ЗаполнитьОбъектПоОбъектуXDTO(Прокси, Объект, ОбъектXDTO, Правило,
	...
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОбъектXDTO, "documentType")
			И ИнтеграцияС1СДокументооборот.СвойствоСуществует(ОбъектXDTO.documentType, "isAPaymentRequest")
			...
			Если ПравилоРеквизита.Вариант = ВариантРеквизит() Тогда
				...
				Для НомерЗначения = 0 По ЗначенияСвойства.Количество() - 1 Цикл
					...
					Если ТипЗнч(ЗначениеСвойства) = Тип("ОбъектXDTO") Тогда
						ТипРеквизита = ИнтеграцияС1СДокументооборот.ПодходящийТип(
...
Процедура ЗаполнитьСтруктуруРеквизитовОбъектаДОПоПравилу(Источник, СтруктураРеквизитов, Правило,
	...
	Для Каждого РеквизитОбъектаДО Из РеквизитыОбъектаДО Цикл
		...
		НайденныеПравилаРеквизитов = ДанныеПравила.ПравилаЗаполненияРеквизитовДО.НайтиСтроки(ПараметрыОтбора);
		Если НайденныеПравилаРеквизитов.Количество() = 0 Тогда
			Если РеквизитОбъектаДО.Таблица = "" И ТипЗнч(НеОбновлятьПоПравилам) = Тип("Массив") Тогда
				Если РеквизитОбъектаДО.ЭтоТаблица Тогда
...
//
Процедура ЗаполнитьПечатныеФормы(Источник, Файлы, Правило, ТолькоОбновление = Ложь, ПомещатьВоВременноеХранилище = Ложь,
	...
	Для Каждого МенеджерПечати Из МенеджерыПечати Цикл
		Отбор = Новый Структура("Идентификатор, МенеджерПечати",
			МенеджерПечати.ИмяКоманды,
			МенеджерПечати.МенеджерПечати);
		СтрокиКомандыПечати = КомандыПечати.НайтиСтроки(Отбор);
		Если СтрокиКомандыПечати.Количество() > 0 Тогда
			МенеджерПечати.ИмяКоманды = "КомандаПечати"
				+ Формат(КомандыПечати.Индекс(СтрокиКомандыПечати[0]),"ЧН=0; ЧГ=0");
		Иначе
	...
	Для Каждого МенеджерПечати Из МенеджерыПечати Цикл
		...
		Для Каждого КомандаМенеджера Из МакетыМенеджера Цикл
			...
			ИнтеграцияС1СДокументооборот.ЗаполнитьПечатныеФормы(
...
Функция СторонаЗаявкиНаОплатуXDTO(ОбъектXDTO, ПредопределенноеИмяСтороны)
	...
	УстановленаТЧСтороны = ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОбъектXDTO, "partyRows");
	
	Если УстановленаТЧСтороны Тогда
		Для Каждого СторонаXDTO Из ОбъектXDTO.partyRows.rows Цикл
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(СторонаXDTO, "description")
					И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(СторонаXDTO.description, "predefinedName")
...
Процедура ЗаполнитьДанныеЗаявкиНаОплату(СтруктураРеквизитов)
	...
	Если (СтруктураРеквизитов.ПолучательТип = "DMCorrespondent"
		Или СтруктураРеквизитов.ПолучательТип = ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаИС("DMCorrespondent"))
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ИнтеграцияС1СДокументооборотПереопределяемый.ПриДобавленииОбработчиковОбновления(Обработчики);
