...
&НаКлиенте
Процедура гкс_ЗадачиОтделаПриИзменении(Элемент)
...
Процедура ОбновитьСписокЗадачНаСервере()
	...
	Попытка
		Прокси = ИнтеграцияС1СДокументооборотПовтИсп.ПолучитьПрокси();
		ЗадачиXDTO = гкс_ПолучитьЗадачиРасширенный(Прокси, Ложь, ЭтотОбъект.гкс_ЗадачиОтдела);
...
&НаСервере
Процедура гкс_ДобавитьРеквизитыФормы()
	
	МассивРеквизитов = Новый Массив;
	
	МассивРеквизитов.Добавить(
		Новый РеквизитФормы("гкс_ЗадачиОтдела", Новый ОписаниеТипов("Булево"), ,"Задачи отдела"));
...
Процедура гкс_ДобавитьЭлементыформы()
	...
	НовыйЭлемент = Элементы.Вставить(
		"гкс_ГруппаОтбор", 
		Тип("ГруппаФормы"), 
		Элементы.ГруппаЗадачи,
		Элементы.Задачи);
	НовыйЭлемент.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НовыйЭлемент.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	
	ВидимостьЭлемента = гкс_Интеграция1СДокументооборотПовтИсп.ВидимостьОтбораЗадачиОтдела();
	
	НовыйЭлемент = Элементы.Добавить("гкс_ЗадачиОтдела", Тип("ПолеФормы"), Элементы["гкс_ГруппаОтбор"]);
	НовыйЭлемент.Вид 			= ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ВидФлажка 		= ВидФлажка.Выключатель;
	НовыйЭлемент.ПутьКДанным 	= "гкс_ЗадачиОтдела";	
	НовыйЭлемент.Видимость 		= ВидимостьЭлемента;
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "гкс_ЗадачиОтделаПриИзменении");
		
КонецПроцедуры

&НаСервереБезКонтекста
Функция гкс_ПолучитьЗадачиРасширенный(Прокси, Выполненные, ЗадачиОтдела = Ложь)
	
	СписокУсловий = ИнтеграцияС1СДокументооборот.СоздатьОбъект(Прокси, "DMObjectListQuery");

	Условие = ИнтеграцияС1СДокументооборот.СоздатьОбъект(Прокси, "DMObjectListCondition");
	Условие.property = "byUser";
	Если ЗадачиОтдела Тогда
		Условие.property = "byDepartment";
	КонецЕсли;
	Условие.value = Истина;
