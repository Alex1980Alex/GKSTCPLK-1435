...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗаголовокУстановлен Тогда
		ИнтеграцияС1СДокументооборотКлиент.УстановитьЗаголовокПриОткрытии(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// }На:{
	Если ИмяСобытия = "Запись_ДокументооборотФайл" Тогда
		Если ЭтотОбъект.гкс_ИдентификацияФайловПоИД Тогда
			гкс_ПрочитатьИОбновитьСписокФайловПоИД();
		Иначе
			ПрочитатьИОбновитьСписокФайловИПодписей();
		КонецЕсли;
	КонецЕсли;
	...
	Иначе // версия известна
		...
		Иначе
			...
			Если ЭтотОбъект.гкс_ИдентификацияФайловПоИД Тогда
				...
				Тип	= Метаданные.НайтиПоТипу(ТипЗнч(Владелец)).ПолноеИмя();
				гкс_ПрочитатьИОбновитьСписокФайловПоИД();
			Иначе
				ДанныеОбъектаДО = ИнтеграцияС1СДокументооборотВызовСервера.
				ДанныеОбъектаДОПоВнешнемуОбъекту(Владелец);
...
Процедура СоздатьФайлЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандыРедактированияЗавершение", ЭтотОбъект);
	// Гкс+[GMFM-56] Коннова Н.В. Изменено: типовой код выполняется, если не заполнено сво-во гкс_ОбщийВладелецПрисоединенныхФайлов
	Если ЗначениеЗаполнено(ЭтотОбъект["гкс_ОбщийВладелецПрисоединенныхФайлов"]) Тогда 
		гкс_Интеграция1СДокументооборотКлиент.СоздатьФайлСДиска(
		УникальныйИдентификатор,
		ID,
		Тип,
		Строка(Владелец),
		Владелец,
		ЭтотОбъект["гкс_ОбщийВладелецПрисоединенныхФайлов"],
		ОписаниеОповещения,
		(СостояниеРазрешаетДобавлениеСканКопии И Не СостояниеРазрешаетДобавлениеФайла));
	Иначе
...
#Область ИзменениеФормыГКС

&НаСервере
Процедура гкс_ДобавитьИзменитьРеквизитыФормы()
	
	МассивРеквизитов	= Новый Массив;
	МассивРеквизитов.Добавить(Новый РеквизитФормы("гкс_ИдентификацияФайловПоИД", Новый ОписаниеТипов("Булево")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("гкс_ОбщийВладелецПрисоединенныхФайлов", Новый ОписаниеТипов)); 
...
Процедура гкс_ИнициализироватьРеквизитыФормы()
	...
	Если Метаданные.ОпределяемыеТипы.гкс_ИнтеграцияС1СДокументоборотСсылкиПрисоединенныеФайлы.Тип.СодержитТип(
		ТипЗнч(Владелец)) Тогда		
		ЭтотОбъект.гкс_ИдентификацияФайловПоИД	= Истина;		
	КонецЕсли;
	Параметры.Свойство("гкс_ОбщийВладелецПрисоединенныхФайлов", ЭтотОбъект["гкс_ОбщийВладелецПрисоединенныхФайлов"]);
...
//
&НаСервере
Процедура гкс_ПрочитатьИОбновитьСписокФайловПоИД()
