...
//
Процедура ВыбратьЗначениеПоТипуОбъекта(ОписаниеОповещения, ТипОбъектаВыбора, Отбор = Неопределено, ВыбранноеЗначение = "",
	...
	Если ИнтеграцияС1СДокументооборотКлиентПовтИсп.ДоступенФункционалВерсииСервиса("1.4.6.1")
...
Процедура ВыполнитьПодключаемуюКомандуИнтеграцииЗавершение(Результат, Параметры) Экспорт
	...
	ИначеЕсли  Команда.Имя = "ИнтеграцияС1СДокументооборотом_Файлы" Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		ОткрытьПрисоединенныеФайлы(Ссылка, Форма, Истина);
...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения, ОткрытьФорму = Ложь) Экспорт
	...
	Если Правила.Количество() = 0 Тогда
		...
		СозданныеПравила = ИнтеграцияС1СДокументооборотВызовСервера.СоздатьПравилаИнтеграцииАвтоматически(ОбъектИС);
...
Процедура ПроверитьПодключение(ОписаниеОповещения, ФормаВладелец = Неопределено,
	...
	// Гкс+[GMFM-66] Свист П.А. от 24.10.2022. Изменено:{
	// Необходимо выполнить доработанный метод.
	гкс_Интеграция1СДокументооборотКлиент.ПроверитьПодключение(
		ОписаниеОповещения, ФормаВладелец, ИмяОбработчика, ПовторноПоказыватьОкноАвторизации);
	Возврат;
	// } Гкс-
...
Процедура ХранилищеОбщихНастроекСохранитьМассив(МассивСтруктур, НужноОбновитьПовторноИспользуемыеЗначения = Ложь) Экспорт
	...
	ИнтеграцияС1СДокументооборотКлиентПереопределяемый.ПриСохраненииНастроекПользователя(
		МассивСтруктур,
...
//
// Возвращаемое значение:
//   Структура, Неопределено - структура отбора, накладываемого перед предъявлением пользователю
//
Функция ОтборПриВыбореСвязанногоОбъекта(СвязываемыйОбъект, ТипОбъектаДокументооборота) Экспорт
	
	Отбор = Неопределено;
	
	ИнтеграцияС1СДокументооборотКлиентПереопределяемый.ПриУстановкеОтбораПриВыбореСвязанногоОбъекта(
		СвязываемыйОбъект,
...
Процедура СохранитьАвторизацияПредложена() Экспорт
	...
	ИнтеграцияС1СДокументооборотКлиентПереопределяемый.ПриСохраненииПредложенияАвторизации();
...
Функция АвторизацияПредложена() Экспорт
	...
	ИнтеграцияС1СДокументооборотКлиентПереопределяемый.ПриОпределенииПредложенияАвторизации(Результат);
...
//
// Параметры:
//   ОписаниеФайла - Массив из Структура:
...
//   Имя - Строка - имя файла.
//   Расширение - Строка - расширение файла.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - уникальный идентификатор вызвавшей формы.
...
Процедура ПолучениеФайлаНачало(Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотКлиентПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
		...
		Если СведенияОРедактировании.Сохранен И Параметры.Тип <> "DMFileVersion" Тогда
			// конкретная версия? проверка не нужна.
...
//
// Параметры:
//   Результат - Строка, Неопределено - "ВзятьИзДокументооборота" или "ИспользоватьСуществующий",
...
//     * УникальныйИдентификаторФормы - УникальныйИдентификатор - уникальный идентификатор вызвавшей формы.
//     * НаЧтение - Булево - Истина, если файл открывается для чтения, и Ложь, если для редактирования.
//         Для версий файлов значение параметра равно только Ложь.
//     * ОписаниеОповещения - ОписаниеОповещения, Неопределено - вызывается при успешном открытии файла.
//         При значении Неопределено вызов не требуется.
...
//     * ПолныеИменаФайлов - Массив из Строка
//     * Файлы - Массив из Структура:
//         ** Идентификатор - Строка - идентификатор файла в ДО.
//         ** Имя - Строка - имя файла.
//         ** Расширение - Строка - расширение файла.
...
Процедура ПолучениеФайлаПослеПолученияИзДокументооборота(Параметры) Экспорт
	...
	НачатьПолучениеФайлов(
		Новый ОписаниеОповещения(
			"ОбработатьФайлПослеПолученияИзДокументооборота", ИнтеграцияС1СДокументооборотКлиент, Параметры,
			"ПолучитьФайлыНачало", ИнтеграцияС1СДокументооборотКлиент),
...
//
// Параметры:
//   ПолучаемыеФайлы - Массив из ОписаниеПередаваемогоФайла
//   Параметры - Структура:
//     * Идентификатор - Строка - идентификатор файла в ДО.
//     * Тип - Строка - DMFile или DMFileVersion.
//     * Имя - Строка - имя файла.
...
//
Процедура ЗакончитьРедактированиеФайла(Идентификатор, Имя, Расширение, УникальныйИдентификаторФормы,
...
//     * Расширение - Строка - расширение редактируемого файла.
//     * УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор формы-владельца.
//     * ОписаниеОповещения - ОписаниеОповещения, Неопределено - вызывается при успешном завершении, если параметр
//         равен Неопределено, то вызывать ничего не нужно.
//     * СведенияОРедактировании - см. ИнтеграцияС1СДокументооборотВызовСервера.ПолучитьСведенияОРедактированииФайла.
...
//
// Параметры:
//   Результат - Массив из Строка, Неопределено - массив с именем файла, если файл выбран, или
//     Неопределено, если пользователь отказался от выбора.
//   Параметры - Структура:
//     * Идентификатор - Строка - идентификатор файла в ДО.
//     * Тип - Строка - DMFile или DMFileVersion.
...
//
// Параметры:
//   Результат - Строка, Неопределено - "ВзятьИзДокументооборота" - взять файл из ДО, "ИспользоватьСуществующий" - взять
//     файл из каталога временных, Неопределено - ничего не делать.
//   Параметры - Структура:
//     * Идентификатор - Строка - идентификатор файла в ДО.
//     * Тип - Строка - DMFile или DMFileVersion.
//     * Имя - Строка - имя файла.
...
//     * Размер - Число - размер файла.
//     * ДатаМодификацииУниверсальная - Дата - универсальные дата и время изменения файла.
//     * ОставшиесяФайлы - Массив из Структура:
//         ** Идентификатор - Строка
//         ** Тип - Строка
//         ** Имя - Строка
//         ** Расширение - Строка
//     * РежимДиалогаВыбораФайла - РежимДиалогаВыбораФайла
//
Процедура СохранитьКакПослеВыбораИсточника(Результат, Параметры) Экспорт
	...
	Если Результат = "ВзятьИзДокументооборота" Тогда
		...
		НачатьПолучениеФайлов(
			Новый ОписаниеОповещения(
				"ПродолжитьСохранитьКакПослеВыбораИсточника", ИнтеграцияС1СДокументооборотКлиент, Параметры),
			ПолучаемыеФайлы,, Ложь);
...
//   ПолныйПуть - Строка - полный путь к файлу, выбранному пользователем.
//   Параметры - Структура:
//     * Идентификатор - Строка - Идентификатор файла ДО.
//     * Имя - Строка - имя редактируемого файла.
//     * Расширение - Строка - расширение редактируемого файла.
...
//         равен Неопределено, то вызывать ничего не нужно.
//
Процедура ОбновитьИзФайлаНаДискеПослеПомещенияФайлаБезРасширения(Результат, Адрес, ПолныйПуть, Параметры) Экспорт
...
//
// Параметры:
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - уникальный идентификатор управляемой формы объекта.
//   ID - Строка - идентификатор владельца файла в Документообороте.
//   Тип - Строка - тип XDTO владельца файла в Документообороте.
...
Процедура СоздатьФайлСДиска(УникальныйИдентификаторФормы, ID, Тип, Представление,
	Владелец = Неопределено, ОписаниеОповещения = Неопределено, ЯвляетсяСканКопией = Ложь) Экспорт
...
//
// Параметры:
//   ВыбранныеФайлы - Массив, Неопределено - имя выбранного файла, или Неопределено, если пользователь
//     отказался от выбора.
//   Параметры - Структура:
//     * ID - Строка - идентификатор владельца файла в Документообороте.
//     * Тип - Строка - тип XDTO владельца файла в Документообороте.
...
//   ОписаниеОповещения - ОписаниеОповещения - оповещение, которое будет вызвано с передачей массива
//     идентификаторов файлов.
//
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	РасширениеФайлаВСписке = ИнтеграцияС1СДокументооборотКлиентСервер.РасширениеФайлаВСписке(
		ОбщиеНастройки.СписокРасширенийТекстовыхФайлов, РасширениеИмениФайла);
	
	Если РасширениеФайлаВСписке Тогда
		Возврат ИнтеграцияС1СДокументооборотКлиентСервер.ИзвлечьТекстИзТекстовогоФайла(ПолноеИмяФайла,
			Неопределено,
			Отказ);
	КонецЕсли;
...
#КонецЕсли
	...
	Если ПустаяСтрока(ИзвлеченныйТекст) Тогда
		...
		Если РасширениеФайлаВСписке Тогда
			Возврат ИнтеграцияС1СДокументооборотКлиентСервер.ИзвлечьТекстOpenDocument(ПолноеИмяФайла, Отказ);
		КонецЕсли;
...
Функция РазложитьПолноеИмяФайла(Знач ПолноеИмяФайла, ЭтоПапка = Ложь) Экспорт
	...
	ИнтеграцияС1СДокументооборотКлиентПереопределяемый.ПриРазложенииПолногоИмениФайла(СтруктураИмениФайла);
...
Процедура ПодписатьФайл(ИдентификаторФайла, ИмяФайла, Редактируется, Зашифрован, ОписаниеФайла, ДанныеПодписейФайла) Экспорт
	...
	Если Редактируется Тогда
		ПоказатьПредупреждение(,
			ИнтеграцияС1СДокументооборотКлиентСервер.СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла());
		Возврат;
	КонецЕсли;
	
	Если Зашифрован Тогда
		ПоказатьПредупреждение(,
			ИнтеграцияС1СДокументооборотКлиентСервер.СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла());
		Возврат;
	КонецЕсли;
...
//   УникальныйИдентификатор - УникальныйИдентификатор - уникальный идентификатор экземпляра формы.
...
//   Параметры - Структура: см. ИнтеграцияС1СДокументооборотКлиент.ПроверитьПодписи
//
...
//
Процедура НачатьДобавлениеЭПИзФайла(СвойстваФайла, ОписаниеОповещенияОЗакрытии = Неопределено) Экспорт
...
// Параметры:
//   ИдентификаторФайла - Строка - идентификатор объекта Документооборота.
//   ТаблицаПодписей - ДанныеФормыКоллекция:
//     * АдресПодписи - Строка
//     * АдресСертификата - Строка
//     * ДатаПодписи - Дата
//     * ДатаПодписиИКомментарий - Строка
...
Процедура НачатьДобавлениеЭПИзФайлаПустойОбработчик(Результат, Параметры) Экспорт
	...
	Возврат; // в этом обработчике не нужны действия
...
//     * Расширение - Строка - расширение файла.
//     * УникальныйИдентификаторФормы - УникальныйИдентификатор - уникальный идентификатор вызвавшей формы.
//     * ОписаниеОповещения - ОписаниеОповещения, Неопределено - вызывается при успешном завершении, или
