...
Процедура ЗаполнитьРеквизитыПоСКД(Реквизиты, Предмет, ПараметрыШаблона) Экспорт
	...
	// Гкс+[GKSTCPLK-645] a.belov от 25.12.2023. Изменено:{
	...
	Если ТипЗнч(ПараметрыШаблона.ВладелецШаблона) = Тип("СправочникСсылка.гкс_ПодпискиНаРассылкиИОповещения") Тогда
		МакетСКД = Справочники.гкс_ПодпискиНаРассылкиИОповещения.ПолучитьМакет(ИмяМакета);
	Иначе		
		МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(МетаданныеОбъекта.ПолноеИмя());
		МакетСКД = МенеджерОбъекта.ПолучитьМакет(ИмяМакета);
	КонецЕсли;
	// } Гкс-[GKSTCPLK-645] a.belov от 25.12.2023.
