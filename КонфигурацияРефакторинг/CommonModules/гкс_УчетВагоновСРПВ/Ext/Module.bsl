...
// Гкс+[GKSTCPLK-1435] Терлецкий АВ 15.01.2025 Возвращает текст запроса для подбора лабораторного анализа
// Возвращает текст запроса для подбора лабораторного анализа в зависимости от функциональной опции
...
Функция ТекстЗапросаЛабАнализ()
	...
	// ЗАКОММЕНТИРОВАННЫЙ КОД: Логика выбора стратегии запроса была дублирована в нескольких модулях
	// Теперь используется единый механизм через централизованный модуль, что упрощает сопровождение
	// Функции ТекстаЗапросаЛабАнализПоВходномуКонтролю() и ТекстаЗапросаЛабАнализПоКомпозиту() оставлены для совместимости
