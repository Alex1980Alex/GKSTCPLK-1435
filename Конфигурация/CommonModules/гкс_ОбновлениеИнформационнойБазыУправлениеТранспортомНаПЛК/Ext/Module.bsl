////////////////////////////////////////////////////////////////////////////////
// Обновление информационной базы библиотеки УправлениеТранспортомНаПЛК.
//
////////////////////////////////////////////////////////////////////////////////
...
Процедура ПриДобавленииПодсистемы(Описание) Экспорт
	...
	Описание.Имя    = "УправлениеТранспортомНаПЛК";
	Описание.Версия = "2025.6.2.0";
	Описание.РежимВыполненияОтложенныхОбработчиков = "Параллельно"; // Запрещено менять
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5a29ea7f-3b0e-4022-973e-abf4a6df6833");
	Обработчик.Процедура = "гкс_ВерсионированиеОбъектов.ОбработатьПервоначальноеЗаполнениеВерсий_1_0_1_1";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение первоначальных версий объектов метаданных, 
	                              |использующих платформенный механизм ""История данных""'");
	
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "гкс_ВерсионированиеОбъектов.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2022.4.1.0";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1a48587f-5462-477f-87d0-2c0f9487fb3d");
	Обработчик.Процедура = "РегистрыСведений.гкс_СвязьРегистрацииИТранспортногоДокумента.ОбработатьДанныеДляЗаполненияТС";
	Обработчик.Комментарий = НСтр("ru = 'Добавить в записи информацию по транспортным средствам.'");
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.гкс_СвязьРегистрацииИТранспортногоДокумента, Документ.гкс_РегистрацияНаПЛК";
	Обработчик.ИзменяемыеОбъекты = "РегистрСведений.гкс_СвязьРегистрацииИТранспортногоДокумента";
	Обработчик.БлокируемыеОбъекты = "РегистрСведений.гкс_СвязьРегистрацииИТранспортногоДокумента";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = 
		"РегистрыСведений.гкс_СвязьРегистрацииИТранспортногоДокумента.ЗарегистрироватьДанныеДляЗаполненияТС";

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2022.4.1.0";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("23cfb2e0-ca02-48eb-9cb9-86c7a5eaa9ab");
	Обработчик.Процедура = "Справочники.гкс_УсловияПоставки.УстановитьУсловиеПоставкиПогрузкаПЛК";
	Обработчик.Комментарий = НСтр("ru = 'Связать предопределенное Условие поставки ПогрузкаПЛК с существующим элементом.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "*";
	Обработчик.Процедура =
		"ПланыВидовХарактеристик.гкс_ПроизвольныеПредопределенныеОбъекты.ЗаполнитьПредопределенныйОбъект";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.НачальноеЗаполнение = Истина;
	ИдентификаторОбработчикаПВХ = "ea9014be-4fd3-4720-a811-ed572933a4d8";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор(ИдентификаторОбработчикаПВХ);
	Обработчик.Комментарий = НСтр("ru = 'Заполнение предопределенного объекта ВидыНоменклатурыЭталоныКачества'");
	...
	Обработчик.РежимВыполнения = "Монопольно"; 
	Обработчик.НачальноеЗаполнение = Ложь;
	ИдентификаторОбработчикаНСН = "a7b8c9d0-1234-5678-9abc-def012345678";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор(ИдентификаторОбработчикаНСН);
	Обработчик.Процедура =  
		"Документы.гкс_НормативнаяСертификацияНоменклатуры.ЗаполнитьТочкуМаршрутаВНормативнойСертификации";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение точки маршрута в документах нормативной сертификации'");	
КонецПроцедуры
...
Процедура СообщитьОНеудачнойОбработке(ИнформацияОбОшибке, Ссылка, ПредставлениеСсылки = Неопределено) Экспорт
	...
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Объект%", 
		?(ПредставлениеСсылки = Неопределено, Ссылка, ПредставлениеСсылки));
		
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
		УровеньЖурналаРегистрации.Предупреждение, Ссылка.Метаданные(),
		Ссылка, ТекстСообщения);
