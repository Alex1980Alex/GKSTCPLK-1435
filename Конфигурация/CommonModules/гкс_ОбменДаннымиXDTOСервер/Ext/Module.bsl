...
// 	   * ХопRMQ - Число - показатель сообщения, характеризующий число приыжков исходного сообщения между базами до 
//	   этого момента 
//     * СквознойНомерRMQ - Строка - идентификатор сообщения,  на основании которого была инициарирована загрузка 
//     объекта.
//     * ЗагруженныеОбъектыСеансаОбмена - Массив из ЛюбаяСсылка - коллекция ссылок на загруженные объекты
//
Функция ИнициализироватьКомпонентыОбмена(НаправлениеОбмена) Экспорт
	...
	КомпонентыОбмена.Вставить("ОшибкиОбработки", Новый Структура);
	КомпонентыОбмена.Вставить("ХопRMQ");
	КомпонентыОбмена.Вставить("СквознойНомерRMQ");
	КомпонентыОбмена.Вставить("ФабрикаXDTO");
	...
	КомпонентыОбмена.Вставить("КлючСообщенияЖурналаРегистрации", "ОбменДанными");
	...
	Если НаправлениеОбмена = "Отправка" Тогда
		...
		// "ВыгрузкаОбъектовПоссылке" - это свойство в компонентах, которое целиком отвечает за механизм 
		// "Выгрузка объектов по ссылке". Предполагается, что настройки могут изменяться динамически во 
		// время выполнения в процедуре гкс_ОчередьСообщенийRMQПереопределяемый.ПередВыгрузкойОбъектовПоСсылке
		ОграниченияВыгрузки = Новый Структура;
		ОграниченияВыгрузки.Вставить("НеВыгружатьПоТипу", Новый Массив);
		ОграниченияВыгрузки.Вставить("НеВыгружатьПоЗначению", Новый Массив);
		
		ВыгрузкаОбъектовПоСсылке = Новый Структура;
		ВыгрузкаОбъектовПоСсылке.Вставить("МаксимальнаяГлубинаВыгрузки", 0);
		ВыгрузкаОбъектовПоСсылке.Вставить("ТекущийУровень", 0);
		ВыгрузкаОбъектовПоСсылке.Вставить("ОграниченияВыгрузки", ОграниченияВыгрузки);

		КомпонентыОбмена.Вставить("ВыгрузкаОбъектовПоСсылке", ВыгрузкаОбъектовПоСсылке);
		...
		КомпонентыОбмена.Вставить("Приложение", гкс_ОчередьСообщенийRMQПовтИсп.ТекущееПриложение());
		КомпонентыОбмена.Вставить("ИмяПОД"); // Для задания возможности выгрузки объекта по определенному ПОД
	...
	Иначе
		...
		ДокументыДляОтложенногоПроведения.Колонки.Добавить("ДополнительныеСвойства");
		...
		КомпонентыОбмена.Вставить("ЗагруженныеОбъектыСеансаОбмена", Новый Массив);
...
Процедура ИнициализироватьТаблицыПравилОбмена(КомпонентыОбмена) Экспорт
	...
	ТаблицаПравилОбработкиДанных = ТаблицаПравилОбработкиДанных(КомпонентыОбмена, 
	                                                            XMLСхема, 
																МенеджерОбмена, 
																НаправлениеОбмена); 
	КомпонентыОбмена.Вставить("ПравилаОбработкиДанных", ТаблицаПравилОбработкиДанных);
	
	ТаблицаПравилКонвертации = ТаблицаПравилКонвертации(КомпонентыОбмена, 
	                                                    XMLСхема, 
														МенеджерОбмена, 
														НаправлениеОбмена, 
														КомпонентыОбмена.ПравилаОбработкиДанных, 
														КомпонентыОбмена.ВерсияФорматаМенеджераОбмена);													
	КомпонентыОбмена.Вставить("ПравилаКонвертацииОбъектов", ТаблицаПравилКонвертации);
	
	ТаблицаПравилКонвертацииПредопределенныхДанных = ТаблицаПравилКонвертацииПредопределенныхДанных(КомпонентыОбмена,
	                                                                                                XMLСхема, 
																									МенеджерОбмена, 
																									НаправлениеОбмена);
	КомпонентыОбмена.Вставить("ПравилаКонвертацииПредопределенныхДанных", ТаблицаПравилКонвертацииПредопределенныхДанных);
	...
	Если Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru = 'Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2'",
				гкс_ОбщегоНазначения.КодОсновногоЯзыка());
			СтрокаСообщения = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ИмяФайлаПротоколаОбмена, ОписаниеОшибки());
...
Функция ЗаписатьВПротоколВыполнения(КомпонентыОбмена,
	...
	Если ТипЗнч(КодОшибки) = Тип("Число") Тогда
		...
		СообщенияОбОшибках = гкс_ОбменДаннымиПовтИсп.СообщенияОбОшибках();
	...
	Если СтруктураЗаписи <> Неопределено Тогда
		...
		Для Каждого Поле Из СтруктураЗаписи Цикл
			...
			КраткоеПредставлениеОшибки  = КраткоеПредставлениеОшибки + Символы.ПС + Отступ + Символы.Таб
				+ гкс_СтроковыеФункцииКлиентСервер.ДополнитьСтроку(Поле.Ключ, Выравнивание, " ", "Справа") + " =  " + Строка(Значение);
	...
	Если ВзвестиФлагОшибок Тогда
		...
		Если КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена = Неопределено Тогда
			КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.Ошибка;
		КонецЕсли;
...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПКО с именем %1'"), Имя);
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	Исключение
		ВызватьИсключение гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Направление: %1.
	...
	Исключение
		ТекстОшибки = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Событие: %1.
	...
	Если КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена = Неопределено Тогда
		КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.Выполнено;
	КонецЕсли;
	
	Если КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.Выполнено
		И (КомпонентыОбмена.ПропускатьОбъектыСОшибкамиПроверкиПоСхеме
		И КомпонентыОбмена.НеВыгруженныеОбъекты.Количество() > 0) Тогда
		КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.ВыполненоСПредупреждениями;
	КонецЕсли;
...
Процедура ВыгрузкаОбъектаВыборки(КомпонентыОбмена, Объект, ПравилоОбработки = Неопределено) Экспорт
	...
	ОбъектСсылочногоТипа = (ТипЗнч(Объект) <> Тип("Структура"))
		И гкс_ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(Объект.Метаданные());
		...
		Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
			...
			Если Не ПропуститьОбработку Тогда
				...
				Если ОшибкаПроверкиПоСхеме Тогда
					...
					ВызватьИсключение ОписаниеОшибкиПроверкиПоСхеме.КраткоеПредставление;
			...
			гкс_ОчередьСообщенийRMQПереопределяемый.ПередВыгрузкойОбъектовПоСсылке(КомпонентыОбмена,
			                                                                       СсылкиИзОбъекта,
			                                                                       Объект,
																				   ПравилоОбработки);
			ВыгрузитьОбъектыПоСсылке(КомпонентыОбмена, СсылкиИзОбъекта, Объект);
			
			// 4. Записываем ОбъектXDTO в XML-файл.
			Фабрика.ЗаписатьXML(КомпонентыОбмена.ФайлОбмена, ОбъектXDTO);
		КонецЦикла;
	КонецЕсли;
...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Если Приемник = Неопределено Тогда
		Приемник = Фабрика.Создать(ТипXDTO);
	КонецЕсли;
		...
		Попытка
			...
			Иначе
				...
				ИначеЕсли ТипСвойства = "ОбычноеСвойство" Тогда
					...
					Если ЭтоСсылкаXDTO(СвойствоТипЗначения, КомпонентыОбмена) Тогда // Конвертация ссылки
					...
					Иначе // Конвертация обычного значения.
						ЗначениеXDTO = Фабрика.Создать(СвойствоТипЗначения, ЗначениеСвойства);
					КонецЕсли;
				...
				ИначеЕсли ТипСвойства = "Таблица" Тогда
					...
					ЗначениеXDTO = Фабрика.Создать(Свойство.Тип);
...
Функция ДанныеXDTOИзДанныхИБ(КомпонентыОбмена, Источник, Знач ПравилоКонвертации, СтекВыгрузки = Неопределено) Экспорт
	...
	Если ПравилоКонвертации.ЭтоСсылочныйТип Тогда
		...
		Если Не гкс_ОбщегоНазначения.СсылкаСуществует(Источник.Ссылка) Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	ЗаписатьЗаголовокСообщенияОбмена(ФайлОбмена, ПараметрыЗаголовка, КомпонентыОбмена);
...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, Знач ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если ПравилоКонвертации.ЭтоСсылочныйТип Тогда
		...
		Если ДанныеXDTOСодержатСсылку
			...
			ОригинальныйУИДСтрокой = ДанныеXDTO["Ссылка"].Значение;
	...
	Если Действие = "ПолучитьСсылку" Тогда
		ДанныеXDTO = Новый Структура("КлючевыеСвойства",
			гкс_ОбщегоНазначения.СкопироватьРекурсивно(ДанныеXDTO));
	КонецЕсли;
	...
	Если Действие = "ПолучитьСсылку" Тогда
		ДанныеXDTO = гкс_ОбщегоНазначения.СкопироватьРекурсивно(ДанныеXDTO.КлючевыеСвойства);
	КонецЕсли;
	...
	Если ПолученныеДанныеСсылка <> Неопределено И гкс_ОбщегоНазначения.СсылкаСуществует(ПолученныеДанныеСсылка) Тогда
		ДанныеИБ = ПолученныеДанныеСсылка.ПолучитьОбъект();
	КонецЕсли;
		...
		Если ЭтоПолнаяЗагрузкаОбъекта
			...
			Если ДанныеИБ <> Неопределено Тогда
				...
				Если СтрокаОбъекта <> Неопределено Тогда
					гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(ДанныеИБ, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
				...
				Если ПравилоКонвертации.ДокументМожетПроводиться Тогда
					...
					Если ДанныеДляЗаписиВИБ.Проведен Тогда
						...
						Если Не ДанныеДляЗаписиВИБ.ЭтоНовый()
							И гкс_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДляЗаписиВИБ.Ссылка, "Проведен") Тогда
						...
						ОнлайнПроведение = ДанныеДляЗаписиВИБ.ДополнительныеСвойства.Свойство("ОнлайнПроведение") 
						И ДанныеДляЗаписиВИБ.ДополнительныеСвойства["ОнлайнПроведение"];
						
						Если ОнлайнПроведение Тогда
							ДобавитьВТаблицуДокументовОнлайнПроведения(КомпонентыОбмена.ДокументыДляОтложенногоПроведения, 
							                                           ДанныеДляЗаписиВИБ);
						Иначе							
							ДобавитьВТаблицуОффлайнПроведения(ДанныеДляЗаписиВИБ); 
						КонецЕсли;
...
Процедура ПроизвестиЧтениеДанных(КомпонентыОбмена, ТаблицыДляЗагрузки = Неопределено) Экспорт
	...
	Если НЕ КомпонентыОбмена.ФлагОшибки
		...
		Попытка
			...
			ПрименитьУдалениеОбъектов(КомпонентыОбмена, 
			                          Результаты.МассивОбъектовКУдалению, 
			                          Результаты.МассивЗагруженныхОбъектов);
		...
		КомпонентыОбмена.ЗагруженныеОбъектыСеансаОбмена = 
		                      гкс_ОбщегоНазначения.СкопироватьРекурсивно(Результаты.МассивЗагруженныхОбъектов);
		КомпонентыОбмена.ПомеченныеНаУдалениеОбъекты = 
							  гкс_ОбщегоНазначения.СкопироватьРекурсивно(Результаты.МассивОбъектовКУдалению);
		
		Если НЕ КомпонентыОбмена.ФлагОшибки Тогда
			
			ТипПроблемыОбмена = Справочники
			                        .гкс_ТипыПроблемОбменаДанными
			                        .ОшибкаВыполненияКодаОбработчиковПриПолученииДанных;
			
			Попытка
				КомпонентыОбмена.МенеджерОбмена.ПослеКонвертации(КомпонентыОбмена);
			Исключение
				
				ОписаниеОшибки =  гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				УстановитьПараметрыПроблемыОбменаДанными(КомпонентыОбмена, ТипПроблемыОбмена);
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		ЗаголовокСообщенияXDTO = Фабрика.ПрочитатьXML(ЧтениеXML, Фабрика.Тип(XMLБазоваяСхема(), "Header"));
...
Функция ОбъектXDTOВСтруктуру(КомпонентыОбмена, ОбъектXDTO) Экспорт
	...
	Для Каждого Свойство Из ОбъектXDTO.Свойства() Цикл		
		КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, ОбъектXDTO, Свойство, Приемник);
	КонецЦикла;
	...
	Если Приемник.Свойство("КлючевыеСвойства")
		...
		СформироватьИДОбъектаXDTOПоНеобходимости(Приемник);
	КонецЕсли;
...
Функция СсылкаОбъектаПоУИДОбъектаXDTO(УИДОбъектаXDTO, ТипЗначенияОбъектаИБ, КомпонентыОбмена) Экспорт
	...
	ПубличнаяСсылка = НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, КомпонентыОбмена.Приложение, ТипЗначенияОбъектаИБ);
	...
	СсылкаПоУИД = СсылкаПоУИД(ТипЗначенияОбъектаИБ, УИДОбъектаXDTO, КомпонентыОбмена.Приложение);
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект,, ОтправкаНазад, КомпонентыОбмена.УзелКорреспондента);
	Иначе
		гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект,, ОтправкаНазад);
	КонецЕсли;
...
Процедура ВыполнитьОтложенноеПроведениеДокументов(КомпонентыОбмена) Экспорт
	...
	ДокументыДляОтложенногоПроведения.Свернуть("ДокументСсылка, ДатаДокумента, ДополнительныеСвойства");
	...
	Для Каждого СтрокаТаблицы Из ДокументыДляОтложенногоПроведения Цикл
		...
		Если НЕ гкс_ОбщегоНазначения.СсылкаСуществует(СтрокаТаблицы.ДокументСсылка) Тогда
			Продолжить;
		КонецЕсли;
			
		ВыполнитьПроведениеДокументаПриЗагрузке(КомпонентыОбмена, 
		                                        СтрокаТаблицы.ДокументСсылка,
		                                        СтрокаТаблицы.ДополнительныеСвойства);
...
//
Процедура ВыполнитьПроведениеДокументаПриЗагрузке(КомпонентыОбмена, 
												  ДокументСсылка, 
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ДополнительныеСвойства) Тогда
			...
			гкс_ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ВыполнитьПроведениеДокументаПриЗагрузке", 
			...
			гкс_ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Объект.ДополнительныеСвойства, 
			                                                    ...
			                                                    Истина);
		КонецЕсли;
		ДополнитьСвойстваОбъектаИсключаемымиПолучателямиИзКомпонентОбмена(Объект, КомпонентыОбмена);
		...
		гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект, Ложь, Ложь);
	...
	Исключение
		...
		ОписаниеОшибки = ПолноеОписаниеОшибкиОбмена(ОписаниеОшибки());		
		УстановитьПараметрыПроблемыОбменаДанными(КомпонентыОбмена, ТипПроблемыОбмена, ПредставлениеОбъекта);
...
Функция ОтменитьПроведениеОбъектаВИБ(Объект, Отправитель) Экспорт
	...
	//
	// Устанавливаем режим загрузки данных для объекта.
	гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект, Истина, Ложь, Отправитель);
	...
	Попытка
		...
		гкс_ОбменДаннымиСервер.УдалитьДвиженияУДокумента(Объект);
...
Функция ПроверитьФорматСообщенияОбмена(ЧтениеXML, КомпонентыОбмена) Экспорт
...
Функция ВерсияФорматаОбменаПриВыгрузке(Знач Получатель) Экспорт
	...
	Результат = гкс_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Получатель, "ВерсияФорматаОбмена");
...
// 
Функция ТаблицаПравилОбработкиДанных(КомпонентыОбмена, XMLСхема, МенеджерОбмена , НаправлениеОбмена)
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если НаправлениеОбмена = "Получение" Тогда
			...
			ТипXDTO = Фабрика.Тип(XMLСхема, ПОД.ОбъектВыборкиФормат);
...
Функция ТаблицаПравилКонвертации(КомпонентыОбмена, XMLСхема, МенеджерОбмена , НаправлениеОбмена, ПравилаОбработкиДанных, ВерсияФорматаМенеджераОбмена)
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если ЗначениеЗаполнено(ПравилоКонвертации.ОбъектФормата) Тогда
			ПравилоКонвертации.ТипXDTO = Фабрика.Тип(XMLСхема, ПравилоКонвертации.ОбъектФормата);
		...
		Если ПравилоКонвертации.ОбъектДанных <> Неопределено Тогда
			...
			ПравилоКонвертации.МенеджерОбъекта            = гкс_ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПравилоКонвертации.ПолноеИмя);
			
			ПравилоКонвертации.ЭтоРегистр                = гкс_ОбщегоНазначения.ЭтоРегистр(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоДокумент               = гкс_ОбщегоНазначения.ЭтоДокумент(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоСправочник             = гкс_ОбщегоНазначения.ЭтоСправочник(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоПеречисление           = гкс_ОбщегоНазначения.ЭтоПеречисление(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоПланВидовХарактеристик = гкс_ОбщегоНазначения.ЭтоПланВидовХарактеристик(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоБизнесПроцесс          = гкс_ОбщегоНазначения.ЭтоБизнесПроцесс(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоЗадача                 = гкс_ОбщегоНазначения.ЭтоЗадача(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоПланСчетов             = гкс_ОбщегоНазначения.ЭтоПланСчетов(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоПланВидовРасчета       = гкс_ОбщегоНазначения.ЭтоПланВидовРасчета(ПравилоКонвертации.ОбъектДанных);
			ПравилоКонвертации.ЭтоКонстанта              = гкс_ОбщегоНазначения.ЭтоКонстанта(ПравилоКонвертации.ОбъектДанных);
...
Функция ТаблицаПравилКонвертацииПредопределенныхДанных(КомпонентыОбмена, XMLСхема, МенеджерОбмена , НаправлениеОбмена)
	...
	Для Каждого ПравилоКонвертации Из ПравилаКонвертации Цикл
		...
		ПравилоКонвертации.ТипXDTO = Фабрика.Тип(XMLСхема, ПравилоКонвертации.ТипXDTO);
...
Функция ТребуетсяВыгрузитьОбъектПоСсылке(КомпонентыОбмена, ЗначениеСсылки)
	
	ТребуетсяВыгрузить = КомпонентыОбмена.ВыгруженныеОбъекты.Найти(ЗначениеСсылки) = Неопределено
			             И ЗначениеСоответствуетОграничениямВыгрузкиПоСсылке(КомпонентыОбмена, ЗначениеСсылки)
				         И НЕ гкс_ОчередьСообщенийRMQВызовСервера.ОбъектУжеВыгруженПоСсылке(ЗначениеСсылки,
						 КомпонентыОбмена.ПолучательДанныхRMQ)
				         И гкс_ОбщегоНазначения.СсылкаСуществует(ЗначениеСсылки)
				         И НЕ ЗначениеСсылки.ПолучитьОбъект() = Неопределено;
				
	Возврат ТребуетсяВыгрузить;
		
КонецФункции

Процедура ВыгрузитьОбъектыПоСсылке(КомпонентыОбмена, СсылкиИзОбъекта, ОбъектРодитель)
		
	Если НЕ (КомпонентыОбмена.Свойство("ВыгружатьСсылкиИзОбъекта") И КомпонентыОбмена.ВыгружатьСсылкиИзОбъекта) Тогда
		Возврат;
	КонецЕсли;

	МаксимальнаяГлубинаВыгрузки = КомпонентыОбмена.ВыгрузкаОбъектовПоСсылке["МаксимальнаяГлубинаВыгрузки"];
	СледующийУровень = КомпонентыОбмена.ВыгрузкаОбъектовПоСсылке["ТекущийУровень"] + 1;
	
	Если СледующийУровень > МаксимальнаяГлубинаВыгрузки Тогда
		Возврат;
	КонецЕсли;
	
	КомпонентыОбмена.ВыгрузкаОбъектовПоСсылке["ТекущийУровень"] = СледующийУровень;
	
	Для Каждого ЗначениеСсылки Из СсылкиИзОбъекта Цикл
		
		Если НЕ ТребуетсяВыгрузитьОбъектПоСсылке(КомпонентыОбмена, ЗначениеСсылки) Тогда
			Продолжить;
		КонецЕсли;
		
		ОбъектВыборки = ЗначениеСсылки.ПолучитьОбъект();
		ДополнитьВыгружаемыйОбъектПоСсылкам(ОбъектВыборки, ОбъектРодитель);
		
		ВыгрузкаОбъектаВыборки(КомпонентыОбмена, ОбъектВыборки);
		КомпонентыОбмена.ВыгруженныеПоСсылкеОбъекты.Добавить(ЗначениеСсылки);
		
		гкс_ОчередьСообщенийRMQВызовСервера.ДобавитьОбъектВСписокВыгруженныхПоСсылке(ЗначениеСсылки, 
		КомпонентыОбмена.ПолучательДанныхRMQ); 
...
Процедура ДополнитьВыгружаемыйОбъектПоСсылкам(ВыгружаемыйОбъект, ОбъектРодитель)
	
	Перем ДопПараметры;

	Если ТипЗнч(ОбъектРодитель) = Тип("Структура") Тогда
		ДопПараметры = Неопределено;
	Иначе
		ОбъектРодитель.ДополнительныеСвойства.Свойство("ИнтеграционныйАдаптерСодружествоПараметры", ДопПараметры);
	КонецЕсли;
	...
	Если ДопПараметры.Свойство("ВыгружатьДопСвойстваВПодчиненные") 
		И ДопПараметры["ВыгружатьДопСвойстваВПодчиненные"] Тогда
		
		ДопСвойстваРодителя = ОбъектРодитель.ДополнительныеСвойства;
		
		ДопСвойстваВыгружаемыйОбъект = ВыгружаемыйОбъект.ДополнительныеСвойства; 
		ДопСвойстваВыгружаемыйОбъект.Вставить("ИнтеграционныйАдаптерСодружествоПараметры", 
		                                                  Новый Структура());
		
		ДопСвойстваВыгружаемыйОбъект["ИнтеграционныйАдаптерСодружествоПараметры"].Вставить("ДопСвойстваРодителя", 
		                                                                                ДопСвойстваРодителя);
...
Функция ЗначениеСоответствуетОграничениямВыгрузкиПоСсылке(КомпонентыОбмена, ЗначениеСсылки)
	
	ОграниченияВыгрузки = КомпонентыОбмена.ВыгрузкаОбъектовПоСсылке.ОграниченияВыгрузки;
	
	ВыгружатьПоТипу = ОграниченияВыгрузки.НеВыгружатьПоТипу.Найти(ТипЗнч(ЗначениеСсылки)) = Неопределено;
	ВыгружатьПоЗначению = ОграниченияВыгрузки.НеВыгружатьПоЗначению.Найти(ЗначениеСсылки) = Неопределено;

	Возврат (ВыгружатьПоТипу И ВыгружатьПоЗначению);
	
КонецФункции

#КонецОбласти

Функция ЭтоСсылкаXDTO(Знач Тип, КомпонентыОбмена)
	
	Фабрика = КомпонентыОбмена.ФабрикаXDTO;
	Возврат Фабрика.Тип(XMLБазоваяСхема(), "Ref").ЭтоПотомок(Тип);
...
Процедура ЗаписатьУдалениеОбъектаXDTO(КомпонентыОбмена, Ссылка, ТипСсылкиXDTO)
	...
	УИДОбъектаXDTO = РегистрыСведений.гкс_ИдентификаторыСинхронизируемыхОбъектов.ПубличныйИдентификаторПоСсылкеОбъекта(
		КомпонентыОбмена.Приложение, Ссылка);
	...
	XMLСхема = КомпонентыОбмена.XMLСхема;
	ТипXDTO  = Фабрика.Тип(XMLСхема, "УдалениеОбъекта");
	
	Для Каждого Свойство Из ТипXDTO.Свойства[0].Тип.Свойства[0].Тип.Свойства Цикл
		Если Свойство.Тип = ТипСсылкиXDTO Тогда
			
			ЗначениеXDTOЛюбаяСсылка = Фабрика.Создать(Свойство.Тип, УИДОбъектаXDTO);
			
			СсылкаНаОбъект = Фабрика.Создать(ТипXDTO.Свойства[0].Тип.Свойства[0].Тип); // ОбъектXDTO
			СсылкаНаОбъект.Установить(Свойство, ЗначениеXDTOЛюбаяСсылка);
			
			ЛюбаяСсылкаОбъект = Фабрика.Создать(ТипXDTO.Свойства[0].Тип);
			ЛюбаяСсылкаОбъект.СсылкаНаОбъект = СсылкаНаОбъект;
			
			ДанныеXDTO = Фабрика.Создать(ТипXDTO);
			ДанныеXDTO.СсылкаНаОбъект = Фабрика.Создать(ТипXDTO.Свойства[0].Тип);
			ДанныеXDTO.Установить(ТипXDTO.Свойства[0], ЛюбаяСсылкаОбъект);
			Фабрика.ЗаписатьXML(КомпонентыОбмена.ФайлОбмена, ДанныеXDTO);
			Прервать;
...
//
Процедура ВыгрузитьУдаление(КомпонентыОбмена, Ссылка, ПравилоКонвертации = Неопределено) Экспорт
...
Функция КонвертироватьПеречислениеВXDTO(КомпонентыОбмена, ЗначениеПеречисления, ТипПеречисленияXDTO)
	...
	Если ТипЗнч(ЗначениеПеречисления) = Тип("Строка") Тогда
		...
		ЗначениеXDTO = Фабрика.Создать(ТипПеречисленияXDTO, ЗначениеПеречисления);
		
	Иначе
		
		ПравилаКонвертацииПредопределенныхДанных = КомпонентыОбмена.ПравилаКонвертацииПредопределенныхДанных;
		
		Если ТипЗнч(ЗначениеПеречисления) = Тип("Структура") Тогда
			
			ПравилоКонвертации = НайтиПравилоКонвертацииДляЗначенияПоИмени(ПравилаКонвертацииПредопределенныхДанных,
			ЗначениеПеречисления.ИмяПКО);
			КонвертируемоеЗначение = ЗначениеПеречисления.Значение;
		...
		Иначе
			...
			КонвертируемоеЗначение = ЗначениеПеречисления;

		КонецЕсли;
        		
		ЗначениеXDTO = Фабрика.Создать(ТипПеречисленияXDTO,
			ЗначениеПеречисленияXDTO(ПравилоКонвертации.КонвертацииЗначенийПриОтправке, КонвертируемоеЗначение));
...
Функция НайтиПравилоКонвертацииДляЗначенияПоИмени(ПравилаКонвертацииПредопределенныхДанных, ИмяПКПД)
	
	ПравилоКонвертации = Неопределено;
	СтруктураПоиска = Новый Структура("ИмяПКПД", ИмяПКПД);
	НайденныеПравила = ПравилаКонвертацииПредопределенныхДанных.НайтиСтроки(СтруктураПоиска);
	...
	ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
		...
		|Задано более одного правила конвертации с именем для <%1>.'"),
		Строка(ИмяПКПД));
...
Процедура ВыгрузитьПоддерживаемыеОбъектыФормата(Header, ПоддерживаемыеОбъекты, УзелКорреспондента, КомпонентыОбмена)
	...
	ДоступныеТипыОбъектовXDTO = Фабрика.Создать(Фабрика.Тип(XMLБазоваяСхема(), "AvailableObjectTypes"));
	...
	Для Каждого СтрокаПоддерживаемыеОбъекты Из ТаблицаПоддерживаемыеОбъекты Цикл
		...
		Если СоздатьНовыйОбъект Тогда
			ТекущийОбъект = Фабрика.Создать(Фабрика.Тип(XMLБазоваяСхема(), "ObjectType"));
...
Процедура ЗаписатьЗаголовокСообщенияОбмена(ФайлОбмена, ПараметрыЗаголовка, КомпонентыОбмена) Экспорт
	...
	// Элемент <Header>
	Header = Фабрика.Создать(Фабрика.Тип(XMLБазоваяСхема(), "Header"));
	...
	Если ПараметрыЗаголовка.ЭтоОбменЧерезПланОбмена Тогда
		...
		Confirmation = Фабрика.Создать(Фабрика.Тип(XMLБазоваяСхема(), "Confirmation"));
		...
		Если ПараметрыЗаголовка.ОбменДаннымиСВнешнейСистемой Тогда
			...
			ВыгрузитьПоддерживаемыеОбъектыФормата(Header,
				                                  ПараметрыЗаголовка.ПоддерживаемыеОбъекты, 
												  ПараметрыЗаголовка.УзелКорреспондента, 
												  КомпонентыОбмена);
		Иначе
	...
	Фабрика.ЗаписатьXML(ФайлОбмена, Header);
...
Процедура ПроверитьОбъектXDTOПоСхеме(ОбъектXDTO, ТипXDTO, Контекст, Отказ, ОписаниеОшибки)
	...
	ОписаниеОшибки.КраткоеПредставление = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонПользовательскоеСообщениеОбОшибке,
		ТипXDTO.Имя,
		ПользовательскоеПредставление,
	...
	ОписаниеОшибки.ПодробноеПредставление = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонСообщениеОбОшибкеЖР,
		Контекст.НаправлениеОбмена,
		Контекст.ИмяПОД,
		Контекст.ИмяПКО,
	...
	Для Каждого Свойство Из ТипОбъектаXDTO.Свойства Цикл
		Если Не ОбъектXDTO.Установлено(Свойство) Тогда
			Если Свойство.НижняяГраница = 1
				...
				СообщениеОбОшибке = гкс_СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
...
#Область ОбработкаЗагружаемыхОбъектов

// Добавить в таблицу документов онлайн проведения
// 
// Параметры:
//  ДокументыДляОтложенногоПроведения - ТаблицаЗначений - Документы для отложенного проведения:
// * ДокументСсылка - ДокументСсылка -
// * ДатаДокумента - Дата -
//  ДанныеДляЗаписиВИБ - ДокументОбъект - 
Процедура ДобавитьВТаблицуДокументовОнлайнПроведения(ДокументыДляОтложенногоПроведения, ДанныеДляЗаписиВИБ)
	...
	СтрокаТаблицы.ДополнительныеСвойства = ДанныеДляЗаписиВИБ.ДополнительныеСвойства;

КонецПроцедуры

// Добавить в таблицу оффлайн проведения.
// 
// Параметры:
//  ДанныеДляЗаписиВИБ - ДокументОбъект - 
Процедура ДобавитьВТаблицуОффлайнПроведения(ДанныеДляЗаписиВИБ)

	ОписаниеОбъекта = гкс_ОтложеннаяОбработкаОбъектов.СтруктураЗаписиОбъектаОтложеннойОбработки();
	ОписаниеОбъекта.ДатаОбъекта = ДанныеДляЗаписиВИБ.Дата;
	ОписаниеОбъекта.Объект = ДанныеДляЗаписиВИБ.Ссылка;
	ОписаниеОбъекта.Действие = Перечисления.гкс_ДействияОтложеннойОбработки.Проведение;
	ОписаниеОбъекта.ДополнительныеСвойства = ДанныеДляЗаписиВИБ.ДополнительныеСвойства;
	
	гкс_ОтложеннаяОбработкаОбъектов.ЗаписатьИнформациюОбОтложеннойОбработке(ОписаниеОбъекта);
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Попытка
		...
		Если ТипЗнч(ЗначениеXDTO) = Тип("ЗначениеXDTO") Тогда
			...
			Значение = ПрочитатьЗначениеXDTO(КомпонентыОбмена, ЗначениеXDTO);
		...
		ИначеЕсли ТипЗнч(ЗначениеXDTO) = Тип("ОбъектXDTO") Тогда
			...
			ИначеЕсли ЭтоТаблицаОбъекта(Свойство) Тогда
				...
				Для Индекс = 0 По ТабличнаяЧастьXDTO.Количество() - 1 Цикл
					...
					Для Каждого СвойствоСтрокиТЧ Из СтрокаXDTO.Свойства() Цикл
						...
						КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, СтрокаXDTO, СвойствоСтрокиТЧ, СтрокаТЧ);
			...
			ИначеЕсли СтрНайти(ЗначениеXDTO.Тип().Имя, "КлючевыеСвойства") > 0 Тогда
				...
				Для Каждого КлючевоеСвойство Из ЗначениеXDTO.Свойства() Цикл
					...
					КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, ЗначениеXDTO, КлючевоеСвойство, Значение);
			...
			ИначеЕсли СтрНайти(ЗначениеXDTO.Тип().Имя, "ОбщиеСвойства") > 0 Тогда
				Если ТипЗнч(Приемник) = Тип("Структура") Тогда 
					...
					Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
						...
						КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, ЗначениеXDTO, ПодСвойство, ПриемникГруппыСвойств);
				...
				Иначе
					...
					Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
						...
						КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, ЗначениеXDTO, ПодСвойство, Приемник);
			...
			Иначе
				...
				Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
					...
					КонвертацияСвойстваXDTOВЭлементСтруктуры(КомпонентыОбмена, ЗначениеXDTO, ПодСвойство, Приемник, ИмяСвойства);
					Прервать;
	...
	Исключение
		...
		НСтр("ru = 'Ошибка чтения объекта XDTO, имя свойства: <%1>.'"),	ИмяСвойства) + Символы.ПС + Символы.ПС 
		+ ПредставлениеОшибки;
...
Функция ПрочитатьЗначениеXDTO(КомпонентыОбмена, ЗначениеXDTO)
	
	Если ЗначениеXDTO = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если ЭтоСсылкаXDTO(ЗначениеXDTO.Тип(), КомпонентыОбмена) Тогда // Конвертация ссылки
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка конвертации свойства объекта XDTO, имя свойства: <%1>.'"), ПКС.СвойствоКонфигурации)
...
Функция СсылкаОбъектаПоСвойствамОбъектаXDTO(ПравилоКонвертации, 
	ПолученныеДанные, 
	ДанныеXDTOСодержатСсылку, 
	Приложение,
	ОригинальныйУИДСтрокой = "") Экспорт
	...
	Для Каждого ПопыткаПоиска Из ПравилоКонвертации.ПоляПоиска Цикл
		...
		АнализироватьПубличныеИдентификаторы = ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
			...
			И ЗначениеЗаполнено(Приложение);
		...
		Если ПоискЗапросом Тогда
			...
			Для Каждого ПолеПоиска Из ПоляПоиска Цикл
				...
				Если гкс_ОбменДаннымиПовтИсп.ЭтоСтроковыйРеквизитНеограниченнойДлины(ПравилоКонвертации.ПолноеИмя, ПолеПоиска.Ключ) Тогда
			...
			Если АнализироватьПубличныеИдентификаторы Тогда
				...
				Если НЕ ПустаяСтрока(ОригинальныйУИДСтрокой) Тогда
					...
					ТекстСоединения = "	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.гкс_ИдентификаторыСинхронизируемыхОбъектов КАК ПубличныеИдентификаторы
					|	ПО ПубличныеИдентификаторы.Ссылка = Таблица.Ссылка 
					|		И ПубличныеИдентификаторы.Приложение = &Приложение
...
Процедура ЗаполнитьДанныеИБПоПолученнымДанным(ДанныеИБ, ПолученныеДанные, ПравилоКонвертации)
	...
	НеКопироватьДополнительныеСвойства = Неопределено;
	ПолученныеДанные
	   .ДополнительныеСвойства
	   .Свойство("НеКопироватьДополнительныеСвойства", НеКопироватьДополнительныеСвойства);
	
	Если НеКопироватьДополнительныеСвойства = Ложь Тогда
		гкс_ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДанныеИБ.ДополнительныеСвойства, 
		                                                    ПолученныеДанные.ДополнительныеСвойства,
		                                                    Истина); 
	КонецЕсли;
...
Процедура СнятьПометкуУдаленияСПредопределенногоЭлемента(Объект, ТипОбъекта, КомпонентыОбмена)
	...
	Если ПометкаПредопределенный.ПометкаУдаления
		...
		КомпонентыОбмена.СостояниеОбменаДанными.РезультатВыполненияОбмена =
			Перечисления.гкс_РезультатыВыполненияОбмена.ВыполненоСПредупреждениями;
...
Процедура УдалитьВременныеОбъектыСозданныеПоСсылкам(КомпонентыОбмена) Экспорт
	...
	Для Каждого СтрокаТаблицы Из СтрокиОбъектыДляУдаления Цикл
		...
		Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
			...
			гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(ОбъектСозданныйПоСсылке, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
...
Процедура ОтложенноеЗаполнениеОбъектов(КомпонентыОбмена)
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеОбъекты Цикл
		...
		Попытка
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить(гкс_ОбщегоНазначения.ИмяТаблицыПоСсылке(СтрокаТаблицы.Объект.Ссылка));
			...
			Если СтруктураПараметров.ОбъектМодифицирован Тогда
				гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
...
Процедура ВыполнитьОтложеннуюЗаписьОбъектов(КомпонентыОбмена)
	...
	Для Каждого ОбъектСоответствия Из КомпонентыОбмена.ОбъектыДляОтложеннойЗаписи Цикл
		...
		Если НЕ гкс_ОбщегоНазначения.СсылкаСуществует(ОбъектСоответствия.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		
		ПредставлениеОбъекта = Строка(ОбъектСоответствия.Ключ);
		ОписаниеОшибки = "";
		...
		Попытка
			...
			// Если при записи объекта (кроме документа) должно формироваться сообщение RMQ в рамках событийной интеграции, 
			// то это будет происходить тогда объект полноценно записывается с прохождением всех проверок. 
			// После отложенного заполнения, при отложеной записи.
			Объект.ДополнительныеСвойства.Удалить("ПропуститьОтправкуВRMQ");
			
			Если ЗначениеЗаполнено(КомпонентыОбмена.СквознойНомерRMQ) Тогда
				Объект.ДополнительныеСвойства.Вставить("СквознойНомерRMQ", КомпонентыОбмена.СквознойНомерRMQ);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(КомпонентыОбмена.ХопRMQ) Тогда
				Объект.ДополнительныеСвойства.Вставить("ХопRMQ", КомпонентыОбмена.ХопRMQ);
			КонецЕсли;
			...
			Иначе
				...
				ОписаниеОшибки = ПолноеОписаниеОшибкиОбмена(ОписаниеОшибки);		
				УстановитьПараметрыПроблемыОбменаДанными(КомпонентыОбмена, ТипПроблемыНезаполненныеРеквизиты, 
		...
		Исключение
			...
			ОписаниеОшибки = ПолноеОписаниеОшибкиОбмена(ОписаниеОшибки());		
			УстановитьПараметрыПроблемыОбменаДанными(КомпонентыОбмена, ТипПроблемыОшибкаЗаписи, ПредставлениеОбъекта);
...
Процедура ПрочитатьСообщениеОбмена(КомпонентыОбмена, Результаты, ТаблицыДляЗагрузки = Неопределено, РежимАнализа = Ложь)
	...
	Фабрика = КомпонентыОбмена.ФабрикаXDTO;
	ТипПроблемыОбмена = Справочники.гкс_ТипыПроблемОбменаДанными.ОшибкаВыполненияКодаОбработчиковПриПолученииДанных; 
	...
	Пока КомпонентыОбмена.ФайлОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		ТипОбъектаXDTO = Фабрика.Тип(КомпонентыОбмена.ФайлОбмена.URIПространстваИмен, КомпонентыОбмена.ФайлОбмена.ЛокальноеИмя);
		ОбъектXDTO     = Фабрика.ПрочитатьXML(КомпонентыОбмена.ФайлОбмена, ТипОбъектаXDTO);
		...
		Если НЕ ЗначениеЗаполнено(ПравилоОбработки) Тогда
			// Так возможно искажение логики, которую закладывают настройкой -  при отсуствии правила явно вызываем исключение
			ТекстИсключения = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Не найдено правило обработки обработки для объекта типа %1'"),
										ТипОбъектаXDTO.Имя);
		...
		Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
			...
			Исключение
				...
				ВызватьИсключение ОписаниеОшибки.ПодробноеПредставление;
			...
			Иначе
				...
				Если КомпонентыОбмена.РежимЗагрузкиДанныхВИнформационнуюБазу
					...
					Исключение
						...
						ОписаниеОшибки = ОписаниеОшибкиПКО(КомпонентыОбмена.НаправлениеОбмена,
														   ПравилоОбработки.Имя,
				...
				ИначеЕсли ТаблицыДляЗагрузки <> Неопределено Тогда
					...
					КлючТаблицыДанных = гкс_ОбменДаннымиСервер.КлючТаблицыДанных(
...
Функция СсылкаПоУИД(ТипЗначенияОбъектаИБ, УИДОбъектаXDTO, Приложение)
	...
	МенеджерОбъектаМетаданных = гкс_ОбщегоНазначения.МенеджерОбъектаПоСсылке(ПустаяСсылка);
	
	НайденнаяСсылка = МенеджерОбъектаМетаданных.ПолучитьСсылку(Новый УникальныйИдентификатор(УИДОбъектаXDTO));
	Если НЕ ЗначениеЗаполнено(Приложение)
		ИЛИ НайденнаяСсылка.Пустая()
		ИЛИ Не гкс_ОбщегоНазначения.СсылкаСуществует(НайденнаяСсылка) Тогда
	...
	Если НЕ РегистрыСведений.гкс_ИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(СтруктураЗаписи) Тогда
...
Функция НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, Приложение, ТипЗначенияОбъектаИБ)
	...
	|	И Таблица.Идентификатор = &Идентификатор");
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли гкс_ОбщегоНазначения.СсылкаСуществует(Выборка.Ссылка) Тогда
			
			Если гкс_ОбщегоНазначения.СсылкаСуществует(НайденнаяСсылка) Тогда
	...
	Если НайденнаяСсылка <> Неопределено
		...
		И НЕ гкс_ОбщегоНазначения.СсылкаСуществует(НайденнаяСсылка) Тогда
	...
	Если УдалитьВсеЗаписи Тогда
		...
		РегистрыСведений.гкс_ИдентификаторыСинхронизируемыхОбъектов.УдалитьЗапись(СтруктураЗаписи, Истина);
...
Процедура ПрочитатьУдаление(КомпонентыОбмена, ОбъектXDTO, МассивОбъектовКУдалению, ТаблицыДляЗагрузки = Неопределено)
	...
	Для Каждого ПравилоКонвертацииИмя Из МассивИменПКО Цикл
		...
		Если ПравилоКонвертации.ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
			ИЛИ ПравилоКонвертации.ВариантИдентификации = "ПоУникальномуИдентификатору" 
			ИЛИ (ПравилоКонвертации.ВариантИдентификации = "ПоПолямПоиска" 
			И НЕ ИменаПравилИсключений().Поиск.Найти(ПравилоКонвертацииИмя) = Неопределено) Тогда
...
Процедура ПрименитьУдалениеОбъектов(КомпонентыОбмена, МассивОбъектовКУдалению, МассивЗагруженныхОбъектов)
	...
	Для Каждого ЭлементКУдалению Из МассивОбъектовКУдалению Цикл
		...
		Объект.ДополнительныеСвойства.Вставить("ПометкаУдаленияПриОбработкеИАС");
...
Процедура УдалитьОбъект(Объект, УдалитьНепосредственно, КомпонентыОбмена)
	...
	Если гкс_ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Предопределенный") Тогда
...
Процедура УстановитьПометкуУдаленияУОбъекта(Объект)
	...
	гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект);
	...
	Попытка
		...
		Для Каждого Движение Из Объект.Движения Цикл
			...
			Если ЕстьИзменения Тогда
				...
				гкс_ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Движение, Истина, Ложь, Отправитель);
				Движение.Записать();
			КонецЕсли;
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных)
	...
	ТипПроблемыОбмена = Справочники.гкс_ТипыПроблемОбменаДанными.ОшибкаВыполненияКодаОбработчиковПриОтправкеДанных;
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД с именем %1'"), Имя);
...
Процедура ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена, Знач ИмяОбработчика)
	...
	Исключение
		...
		ОписаниеОшибки = ПолноеОписаниеОшибкиОбмена(ТекстОшибки);		
		УстановитьПараметрыПроблемыОбменаДанными(КомпонентыОбмена, ТипПроблемыОбмена);
...
Функция РезультатВыполненияОбменаОшибка(РезультатВыполненияОбмена)
	...
	Возврат РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.Ошибка
		Или РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.Ошибка_ТранспортСообщения;
	
КонецФункции

Функция РезультатВыполненияОбменаПредупреждение(РезультатВыполненияОбмена)
	
	Возврат РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.ВыполненоСПредупреждениями
		Или РезультатВыполненияОбмена = Перечисления.гкс_РезультатыВыполненияОбмена.Предупреждение_СообщениеОбменаБылоРанееПринято;
...
Функция ПредставлениеОбъектаДляПротокола(Объект, МетаданныеДляПредставленияСтруктуры = Неопределено)
	...
	Если МетаданныеОбъекта <> Неопределено
		И гкс_ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта)
	...
	Иначе
		...
		ИначеЕсли ТипОбъекта = Тип("Структура") Тогда
			...
			Если Объект.Свойство("Ссылка")
				...
				Если гкс_ОбщегоНазначения.ЭтоСсылка(ТипОбъекта) Тогда
	...
	ПредставлениеОбъекта = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, %2 (%3)'"),
...
Функция ПредставлениеКоллекцииСвойствДляПротокола(КоллекцияСвойств)
	...
	Если КоллекцияСвойств.Свойство("Номер", ЗначениеСвойства)
		...
		Если КоллекцияСвойств.Свойство("Дата", ЗначениеСвойстваДата)
			...
			ЭлементыПредставления.Добавить(
				гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№%1 от %2'"), ЗначениеСвойства, Формат(ЗначениеСвойстваДата, "ДЛФ=D")));
		Иначе
			ЭлементыПредставления.Добавить(
				гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№%1'"), ЗначениеСвойства));
		КонецЕсли;
	...
	ИначеЕсли КоллекцияСвойств.Свойство("Дата", ЗначениеСвойства)
		...
		ЭлементыПредставления.Добавить(
			гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'от %1'"), ЗначениеСвойства));
...
Процедура УстановитьПараметрыПроблемыОбменаДанными(КомпонентыОбмена, ТипПроблемы, ПредставлениеОбъекта = Неопределено)
	
	Если НЕ КомпонентыОбмена.ОшибкиОбработки.Свойство("ТипПроблемы") Тогда		
		КомпонентыОбмена.ОшибкиОбработки.Вставить("ТипПроблемы", ТипПроблемы);	
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПредставлениеОбъекта) 
		И НЕ КомпонентыОбмена.ОшибкиОбработки.Свойство("ПредставлениеОбъекта") Тогда		
		КомпонентыОбмена.ОшибкиОбработки.Вставить("ПредставлениеОбъекта", ПредставлениеОбъекта);		
	КонецЕсли;
...
Функция ОписаниеОшибкиПОД(НаправлениеОбмена, ИмяПОД, ПредставлениеОбъекта, Информация)
	...
	Результат.КраткоеПредставление = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонОписанияОшибки,
		НаправлениеОбмена,
		ИмяПОД,
		ПредставлениеОбъекта,
		КраткоеПредставлениеОшибки(Информация));
	Результат.ПодробноеПредставление = гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонОписанияОшибки,
		НаправлениеОбмена,
		ИмяПОД,
		ПредставлениеОбъекта,
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.'"), Версия);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неправильный формат параметра СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение гкс_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неправильный формат параметра СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
...
Процедура ЗаписатьПриНеобходимостиПубличныйИдентификатор(
	...
	ВариантИдентификации = СокрЛП(ПравилоКонвертации.ВариантИдентификации);
	ПравилоКонвертацииИмя = СокрЛП(ПравилоКонвертации.ИмяПКО);
	...
	Если НЕ (ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
		...
		ИЛИ (ВариантИдентификации = "ПоПолямПоиска" 
		И НЕ ИменаПравилИсключений().Поиск.Найти(ПравилоКонвертацииИмя) = Неопределено))
		ИЛИ НЕ ЗначениеЗаполнено(Приложение) Тогда
	...
	Если ДанныеДляЗаписиВИБ <> Неопределено
		И РегистрыСведений.гкс_ИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(СтруктураЗаписи) Тогда
...
Функция ОбъектПроходитПоФильтруТаблицДляЗагрузки(ТаблицыДляЗагрузки, ТипДанныхПриОтправке, ТипДанныхПриПолучении)
	...
	КлючТаблицыДанных = гкс_ОбменДаннымиСервер.КлючТаблицыДанных(ТипДанныхПриОтправке, ТипДанныхПриПолучении, Ложь);
...
Процедура СформироватьИДОбъектаXDTOПоНеобходимости(ДанныеXDTO)
    
    СтрокаИДОбъекта = Строка(Новый УникальныйИдентификатор);
    
    Если ДанныеXDTO.Свойство("Ссылка") И (НЕ ЗначениеЗаполнено(ДанныеXDTO.Ссылка.Значение) 
        ИЛИ ДанныеXDTO.Ссылка.Значение = "00000000-0000-0000-0000-000000000000") Тогда                            
        
        ДанныеXDTO.Ссылка.Значение = СтрокаИДОбъекта;
        
    КонецЕсли; 
    
    Если ДанныеXDTO.Свойство("КлючевыеСвойства") И ДанныеXDTO.КлючевыеСвойства.Свойство("Ссылка") 
        И (НЕ ЗначениеЗаполнено(ДанныеXDTO.КлючевыеСвойства.Ссылка.Значение) 
        ИЛИ ДанныеXDTO.КлючевыеСвойства.Ссылка.Значение = "00000000-0000-0000-0000-000000000000")Тогда

        ДанныеXDTO.КлючевыеСвойства.Ссылка.Значение = СтрокаИДОбъекта;
        
    КонецЕсли;
    
КонецПроцедуры

Функция ИменаПравилИсключений()
	
	Исключения = Новый Структура;
	Исключения.Вставить("Поиск", Новый Массив);
	
	гкс_ОчередьСообщенийRMQПереопределяемый.ПриОпределенииИменПравилИсключенийПоиска(Исключения["Поиск"]);
...
// Дополнить объект исключаемыми получателями из компонентов обмена.
// 
// Параметры:
//  Объект - ДокументОбъект, СправочникОбъект - 
//  КомпонентыОбмена - Структура - см. гкс_ОбменДаннымиXDTOСервер.ИнициализироватьКомпонентыОбмена 
//
Процедура ДополнитьСвойстваОбъектаИсключаемымиПолучателямиИзКомпонентОбмена(Объект, КомпонентыОбмена)
	
	ИсключаемыеПолучатели = Неопределено;
	Если НЕ (КомпонентыОбмена.Свойство("ИсключаемыеПолучатели", ИсключаемыеПолучатели)
	     И ЗначениеЗаполнено(ИсключаемыеПолучатели)) Тогда
	     Возврат;
	КонецЕсли;
	
	Объект.ДополнительныеСвойства.Вставить("ИсключаемыеПолучатели", ИсключаемыеПолучатели);
