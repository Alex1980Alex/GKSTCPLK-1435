...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
    ...
    ПодпискиНаРассылки = Справочники.гкс_ПодпискиНаРассылкиИОповещения.ПодпискиНаРассылки();
		
	Для Каждого Подписка ИЗ ПодпискиНаРассылки Цикл
		
		Предмет = Настройки.ПредметыШаблонов.Добавить();
		Предмет.Имя = Подписка.Ключ;
		Предмет.Представление = СтрШаблон(НСтр("ru = '%1'"), Подписка.Значение);
		Предмет.Макет = Подписка.Ключ;
...
Процедура ПриПодготовкеШаблонаСообщения(Реквизиты, Вложения, НазначениеШаблона, ДополнительныеПараметры) Экспорт
	...
	Если ПодпискиНаРассылки.Получить(НазначениеШаблона) <> Неопределено Тогда
					
		ШаблоныСообщений.СформироватьСписокРеквизитовПоСКД(Реквизиты, 
			Справочники.гкс_ПодпискиНаРассылкиИОповещения.ПолучитьМакет(НазначениеШаблона));
...
Процедура ПриФормированииСообщения(Сообщение, НазначениеШаблона, ПредметСообщения, ПараметрыШаблона) Экспорт
	...
	ДействующиеПодписки = Справочники.гкс_ПодпискиНаРассылкиИОповещения.ПодпискиНаРассылки(Истина);
				
	Если ДействующиеПодписки.Получить(НазначениеШаблона) <> Неопределено Тогда	
...
Процедура ПриЗаполненииПочтыПолучателейВСообщении(ПолучателиПисьма, НазначениеШаблона, ПредметСообщения) Экспорт
	...
	ДействующиеПодписки = Справочники.гкс_ПодпискиНаРассылкиИОповещения.ПодпискиНаРассылки(Истина);
	Подписка = ДействующиеПодписки.Получить(НазначениеШаблона);
				
	Если Подписка <> Неопределено Тогда	
		гкс_РассылкиИОповещения.ЗаполнитьПолучателейСообщенияПодписки(Подписка, ПолучателиПисьма);		
	КонецЕсли;
