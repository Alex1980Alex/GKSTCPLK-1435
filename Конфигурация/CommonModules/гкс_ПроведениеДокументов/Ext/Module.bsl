...
Процедура ДополнитьЗапросОтраженияДокумента(Запрос, ТекстыЗапроса, ТекстыШаблоновОтражения, ТекстыЗапросовДанныхДокумента, ТекстыШаблоновВременныхТаблиц = Неопределено) Экспорт
	...
	Для каждого КлючИЗначение Из ТекстыЗапросовДанныхДокумента Цикл
		...
		Если ПозицияКлючевогоСлова("ОБЪЕДИНИТЬ", КлючИЗначение.Значение) <> Неопределено Тогда
			...
			|";
			ТекстСозданияВременнойТаблицы = СтрЗаменить(ТекстСозданияВременнойТаблицы, "//&Поместить", СтрШаблон("ПОМЕСТИТЬ %1", ИмяВременнойТаблицы));
...
Функция РегистраторыДляПерепроведения(ТекстыЗапросов, ПолноеИмяРегистра, ПолноеИмяДокумента) Экспорт
	...
	РазделительЗапросов	= гкс_ОбщегоНазначенияПЛК.РазделительЗапросовВПакете();
	...
	ТекстЗапросаФормированияДвижений = СтрЗаменить(ТекстЗапросаФормированияДвижений,
													РазделительЗапросов,
													гкс_ОбщегоНазначенияПЛК.РазделительЗапросовВОбъединении());
...
// 	ТекстыЗапроса - СписокЗначений - Список текстов запросов для проведения документа
...
Функция УчетныеМеханизмыКонфигурации() Экспорт
	...
	МеханизмыКонфигурации.Вставить("ПриемкаПЛК", "гкс_ПриемкаНаПЛКСервер");
	МеханизмыКонфигурации.Вставить("НормативнаяСертификацияНоменклатуры", "гкс_НормативнаяСертификацияНоменклатуры");
	МеханизмыКонфигурации.Вставить("ЭлектроннаяОчередьПриемки", "гкс_ЭлектроннаяОчередьПриемки");
	МеханизмыКонфигурации.Вставить("ЭлектроннаяОчередьОтгрузки", "гкс_ЭлектроннаяОчередьОтгрузки");	
	МеханизмыКонфигурации.Вставить("ЛабораторияИКачество", "гкс_ЛабораторияИКачество");	
	МеханизмыКонфигурации.Вставить("СтатусыДопускаЖДТранспорта", "РегистрыСведений.гкс_СтатусыДопускаВагоновКВскрытию");
	МеханизмыКонфигурации.Вставить("ДокументальноеОформление", "гкс_ДокументальноеОформление");
	МеханизмыКонфигурации.Вставить("УчетФГИСЗерно", "гкс_УчетФГИСЗерно");
		
	Если ПолучитьФункциональнуюОпцию("гкс_ИспользоватьИнтеграциюСРПВ") Тогда
		МеханизмыКонфигурации.Вставить("УчетВагоновСРПВ", "гкс_УчетВагоновСРПВ");
		МеханизмыКонфигурации.Вставить("ЖДНакладныеВагона", "РегистрыСведений.гкс_ЖДНакладныеВагона");
	КонецЕсли;
...
Функция ВыполнитьПакетЗапросовПроведения(Запрос, ТекстыЗапроса)
	...
	Для Каждого ТекстЗапроса Из ТекстыЗапроса Цикл
		...
		МассивТекстовЗапросов.Добавить(ТекстЗапроса.Значение + гкс_ОбщегоНазначенияПЛК.РазделительЗапросовВПакете());
...
Функция ПолучитьИспользуемыеРегистры(Регистратор, СписокРегистров)
	...
	Для каждого Регистр Из СписокРегистров Цикл
		...
		Если СчетчикТаблиц = МаксимумТаблицВЗапросе Или ОсталосьРегистров = 0 И СчетчикТаблиц <> 0 Тогда
			...
			Запрос.Текст  = СтрСоединить(ТекстыЗапроса, гкс_ОбщегоНазначенияПЛК.РазделительЗапросовВОбъединении());
			СчетчикТаблиц = 0;
...
Процедура ВыполнитьКонтрольРезультатовПроведения(Механизмы, Документ, МенеджерВременныхТаблиц, Отказ)
	...
	РезультатыКонтроля = гкс_ОбщегоНазначенияПЛК.ВыгрузитьРезультатыЗапроса(Запрос, ТекстыЗапроса,, Истина);
...
Процедура ВыполнитьЗаменуПоПолномуСоответствию(Текст, СтрокаПоиска, СтрокаЗамены, ТолькоПервый = Ложь) Экспорт
	...
	ТегДляЗамены = "%ТэгЗамены%";
	РазметитьТекстДляЗаменыПоПолномуСоответствию(Текст, СтрокаПоиска, ТегДляЗамены, ТолькоПервый);
	Текст = СтрЗаменить(Текст, ТегДляЗамены, СтрокаЗамены);
	
КонецПроцедуры

Процедура РазметитьТекстДляЗаменыПоПолномуСоответствию(Текст, СтрокаПоиска, ТегДляЗамены, ТолькоПервый = Ложь)
	...
	Пока ПозицияСтрокиПоиска <> 0 Цикл
		...
		Если ВыполнятьЗамену Тогда
			...
			Если СтрокиДляЗамены.Получить(ПолнаяСтрокаПоиска) = Неопределено Тогда
				ПолнаяСтрокаЗамены = СтрШаблон("%1%2%3", ПредыдущийСимвол, ТегДляЗамены, СледующийСимвол);
...
Процедура ВыполнитьЗаменыВСекцииВЫБРАТЬ(ТекстЗапроса, ИмяТаблицыДанных, РезультатРазбораТекстаЗапросаДанных)
	...
	Для каждого ПолеВыборки Из РезультатРазбораТекстаЗапросаДанных.ПоляВыборки Цикл
		...
		ТегДляЗамены = "%" + ПолеВыборки.Ключ + "%";
		РазметитьТекстДляЗаменыПоПолномуСоответствию(ТекстЗапроса, СтрокаПоиска, ТегДляЗамены);
		ТегиДляЗамены.Вставить(ТегДляЗамены, ПолеВыборки.Значение);
	КонецЦикла;
	Для каждого КлючИЗначение Из ТегиДляЗамены Цикл
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
