...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Метаданные.Обработки.Найти("ЗагрузкаКурсовВалют") <> Неопределено Тогда
		КодыВалютДляЗагрузки = Обработки["ЗагрузкаКурсовВалют"].КодыВалютЗагружаемыхИзИнтернета();
		КодыВалютЗагружаемыхИзИнтернета.ЗагрузитьЗначения(КодыВалютДляЗагрузки);
	КонецЕсли;
	
	Элементы.КурсВалютыЗагружаетсяИзИнтернета.Доступность = Истина;
