...
Процедура ШаблонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот.Форма.ВыборИзСписка", ПараметрыФормы, ЭтотОбъект,,,,
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Иначе // версия известна
		...
		Иначе
			...
			Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("3.0.1.1") Тогда
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	Если Не ЗначениеЗаполнено(Объект.ТипФайловСохраненияПечатныхФорм) Тогда
		Объект.ТипФайловСохраненияПечатныхФорм = ИнтеграцияС1СДокументооборот.
...
Процедура ЗаполнитьРеквизитыПотребителя()
	...
	Реквизиты = Справочники.ПравилаИнтеграцииС1СДокументооборотом.ПолучитьРеквизитыОбъектаИС(Объект.ТипОбъектаИС);
...
Процедура ЗаполнитьПоУмолчаниюПриВыгрузкеНаСервере(НачальноеЗаполнение = Ложь)
	...
	Если СтрокиВидаДокумента.Количество() > 0 Тогда
		...
		Если ВидДокументаДО <> Неопределено И ДоступенФункционалКонтрагенты Тогда
			...
			ВидXDTO = ИнтеграцияС1СДокументооборот.СоздатьObjectID(
				Прокси,
				ВидДокументаДО.ID,
				ВидДокументаДО.type);
