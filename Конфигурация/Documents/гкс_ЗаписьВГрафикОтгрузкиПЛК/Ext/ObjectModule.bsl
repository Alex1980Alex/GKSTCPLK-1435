...
Процедура ПроверитьСвязанныеЗаписиВОчередь(Отказ)
	...
	Если ЕстьЗаписиВОчередиОтгрузки() И Не Отменено Тогда		
...
Функция ДанныеДокумента()
	...
	|ИЗ
	|	Документ.гкс_ЗаписьВГрафикОтгрузкиПЛК КАК ТаблицаДокумента
	|ГДЕ
...
Процедура ПроверитьДоступностьМестВГрафикеИОчереди(Отказ)
	...
	Если НЕ ЕстьСвободноеМестоВГрафике() Тогда
		...
		ТекстСообщения = НСтр("ru = 'На этот период в графике отгрузки уже существуют записи!'");
	...
	Если НЕ ЕстьСвободноеМестоВОчереди() Тогда
		...
		ТекстСообщения = НСтр("ru = 'На этот период в очередь отгрузки уже есть запись!'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Неопределено, "ОкончаниеПериода", "", Отказ);
...
Функция ЕстьСвободноеМестоВОчереди()
	...
	Если ЭтоНовый() Тогда
		Возврат Не ЕстьЗаписиВОчередиОтгрузки();
	КонецЕсли;
...
Функция ЕстьСвободноеМестоВОчередиДляИзмененнойЗаписи()
	...
	Возврат Не РегистрыСведений.гкс_ОчередьОтгрузкиПЛК.ЕстьЗаписьВОчередь(ПараметрыЗаписиВОчередь);
	
КонецФункции

Функция ЕстьЗаписиВОчередиОтгрузки()
	...
	Возврат РегистрыСведений.гкс_ОчередьОтгрузкиПЛК.ЕстьЗаписьВОчередь(ПараметрыЗаписиВОчередь);
...
Функция ЕстьСвободноеМестоВГрафике()
	...
	ДанныеГрафика.Вставить("Номенклатура", Номенклатура);
	ДанныеГрафика.Вставить("НачалоПериода", НачалоПериода);
	ДанныеГрафика.Вставить("ОкончаниеПериода", ОкончаниеПериода);
	
	УстановитьБлокировкуГрафикаПоПараметрам(ДанныеГрафика);
	
	Если НЕ ЭтоНовый() Тогда
		ДанныеГрафика.Вставить("Ссылка", Ссылка);	
	КонецЕсли;
	
	Период = Новый Структура("ДатаНачала, ДатаОкончания", НачалоПериода, ОкончаниеПериода);
	
	Возврат РегистрыСведений.гкс_ГрафикОтгрузкиПЛК.ЕстьСвободноеМесто(Период, Номенклатура, Ссылка);
...
Процедура УстановитьБлокировкуОчередиПоПараметрам(Параметры)
	...
	Если Параметры.Свойство("Периоды") Тогда
		...
		Для Каждого Период Из Параметры.Периоды Цикл
			...
			ДобавитьЭлементБлокировкиОчереди(Блокировка, Параметры.Номенклатура, Диапазон);
		КонецЦикла;	
	Иначе		
...
Процедура ДобавитьЭлементБлокировкиОчереди(Блокировка, Номенклатура, Диапазон)
...
Процедура УстановитьБлокировкуГрафикаПоПараметрам(Параметры)
	
	Диапазон = Новый Диапазон(Параметры.НачалоПериода, Параметры.ОкончаниеПериода);
	
	Блокировка = Новый БлокировкаДанных;
	
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.гкс_ГрафикОтгрузкиПЛК");
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
