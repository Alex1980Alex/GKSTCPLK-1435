...
Процедура ОбработкаОповещения(ИмяСобытия, Параметры, Источник)
	...
	Если ИмяСобытия = "Запись_ДокументооборотДокумент" Или ИмяСобытия = "Запись_ДокументооборотФайл" Тогда
...
Процедура ДеревоПриложенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе // объект ДО
		ИнтеграцияС1СДокументооборотКлиент.ОткрытьОбъект(Строка.ПредметТип, Строка.ПредметID, ЭтотОбъект);
...
Процедура ОткрытьБизнесПроцесс(Команда)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОткрытьОбъект(ПроцессТип, ПроцессID, ЭтотОбъект);
...
Процедура ЗаполнитьФормуЗадачи(ПараметрыЗадачи)
	...
	Ответ = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(Прокси, ПараметрыЗадачи.type, ПараметрыЗадачи.ID, Условия);
	...
	Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтотОбъект, ОбъектXDTO.importance,"Важность");
	Если ОбъектXDTO.Свойства().Получить("state") <> Неопределено Тогда
		Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтотОбъект, ОбъектXDTO.state, "Состояние");
	КонецЕсли;
	...
	Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтотОбъект, ОбъектXDTO.author, "Автор");
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
		...
		Если ОбъектXDTO.Установлено("target") Тогда
			Если Найти(ОбъектXDTO.target.objectID.type,"Document") <> 0 Тогда
				МассивОбъектов.Добавить(
					Новый Структура("ID, type", ОбъектXDTO.target.ObjectID.ID, ОбъектXDTO.target.ObjectID.type));
...
Процедура ЗаполнитьXDTOПоОбъектномуРеквизиту(Прокси, ИмяРеквизита, ИмяСвойстваОбъекта, ОбъектXDTO)
	...
	СвойствоОбъекта.ObjectID = ИнтеграцияС1СДокументооборот.СоздатьObjectID(Прокси,
...
Функция ЗаписатьОбъект(РезультатВыполненияЗадачи = Неопределено)
	...
	Обработки.ИнтеграцияС1СДокументооборот.СформироватьДополнительныеСвойства(Прокси, Задача, ЭтотОбъект);
...
Процедура ОбработкаОповещенияЗаписьПредмета(Параметры)
	...
	Если ДоступнаМультипредметность Тогда // найдем нужную строку и обновим ее
		...
		Для Каждого СтрокаПредмета Из Дерево.Строки Цикл
			Если Параметры.ID = СтрокаПредмета.ПредметID И Не ЗначениеЗаполнено(СтрокаПредмета.Ссылка) Тогда
	...
	Иначе // мультипредметности нет, перезаполним дерево целиком
		...
		Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтотОбъект, ПредметXDTO, "Предмет");
