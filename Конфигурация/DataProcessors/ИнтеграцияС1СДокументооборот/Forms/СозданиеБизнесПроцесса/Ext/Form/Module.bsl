...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступноПолучениеСводки = ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.28.12.CORP");
...
Функция СводкаШаблона(ШаблонID, ШаблонТип)
	...
	Запрос = ИнтеграцияС1СДокументооборот.СоздатьОбъект(Прокси, "DMGetTemplateHintRequest");
	Запрос.template = ИнтеграцияС1СДокументооборот.СоздатьОбъект(Прокси, "DMObjectID");
	Запрос.template.ID = ШаблонID;
	Запрос.template.type = ШаблонТип;
	
	РезультатЗапроса = Прокси.execute(Запрос);
	ИнтеграцияС1СДокументооборот.ПроверитьВозвратВебСервиса(Прокси, РезультатЗапроса);
...
Процедура ЗаполнитьДеревоШаблонов(ПолучатьВидДокумента = Ложь)
	...
	Если ИнтеграцияС1СДокументооборот.СвойствоСуществует(Запрос, "getComplexBusinessProcessTemplateHint") Тогда
	...
	Для Каждого Раздел Из РезультатДеревоШаблонов.BusinessProcessTemplatesTree Цикл
		...
		Если Раздел.name = НСтр("ru = 'Рекомендованные шаблоны'") Тогда
			...
			Для Каждого Шаблон Из Раздел.elements Цикл
				...
				Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Шаблон, "hintHTML") Тогда
					...
					СтрокаРаздела.ПодсказкаПолучена = Истина;
				ИначеЕсли ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Шаблон, "hint") Тогда
					СтрокаРаздела.Подсказка = Шаблон.hint;
					СтрокаРаздела.ПодсказкаПолучена = Истина;
				КонецЕсли;
				СтрокаРаздела.ЭтоЗаголовок = Ложь;
		...
		Иначе
			...
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Раздел, "hintHTML") Тогда
				...
				РазделДерева.ПодсказкаПолучена = Истина;
			ИначеЕсли ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Раздел, "hint") Тогда
				РазделДерева.Подсказка = Раздел.hint;
				РазделДерева.ПодсказкаПолучена = Истина;
			КонецЕсли;
			РазделДерева.ЭтоЗаголовок = Истина;
