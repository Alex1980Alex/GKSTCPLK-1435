...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийПользовательXDTO = ИнтеграцияС1СДокументооборотПовтИсп.ТекущийПользовательДокументооборота();
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.28.12") Тогда
	...
	Если Не ЗначениеЗаполнено(ID)
		...
		Объекты.Добавить(Новый Структура("ID, type", ВидДокументаID, ВидДокументаТип));
		Если ОрганизацияID <> "" И ОрганизацияТип = "DMOrganization" Тогда
			Объекты.Добавить(Новый Структура("ID, type", ОрганизацияID, ОрганизацияТип));
		КонецЕсли;
		Для Каждого Строка Из Стороны Цикл
			Если Строка.СторонаТип = "DMCorrespondent" Тогда
				Объекты.Добавить(Новый Структура("ID, type", Строка.СторонаID, Строка.СторонаТип));
			КонецЕсли;
		КонецЦикла;
		ДанныеОбъектовДО = ИнтеграцияС1СДокументооборот.ПолучитьОбъекты(Прокси, Объекты);
		
		Обработки.ИнтеграцияС1СДокументооборот.ПриИзмененииВидаНаФормеДокумента(ЭтотОбъект, ДанныеОбъектовДО);
		
		Для Каждого Строка Из Стороны Цикл
			legalPrivatePersonID = "";
			Если Строка.СторонаТип = "DMCorrespondent" Тогда
				Для Каждого Элемент Из ДанныеОбъектовДО.objects Цикл
	...
	ИначеЕсли ИмяСобытия = "Документооборот_ВыбратьЗначениеИзСпискаЗавершение" И Источник = ЭтотОбъект Тогда
		...
		ИначеЕсли Параметр = "Получатель" Тогда
			Элементы.АвансовыйОтчет.Видимость = (ПолучательТип = "DMUser"
				Или ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаXDTO(ПолучательТип) = "DMUser");
...
Функция ДанныеОрганизации(ОрганизацияТип, ОрганизацияID)
	...
	ОбъектыXDTO = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(Прокси, ОрганизацияТип, ОрганизацияID);
	Если ОбъектыXDTO.objects.Количество() = 1 Тогда
		ОрганизацияXDTO = ОбъектыXDTO.objects[0];
		Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОрганизацияXDTO, "VATpayer") Тогда
...
Процедура ПолучательОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ПолучательID) Тогда
		ИнтеграцияС1СДокументооборотКлиент.ОткрытьОбъект(ПолучательТип, ПолучательID, Элемент);
	КонецЕсли;
...
Процедура АвансовыйОтчетОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(АвансовыйОтчетID) Тогда
		ИнтеграцияС1СДокументооборотКлиент.ОткрытьОбъект("DMInternalDocument", АвансовыйОтчетID, Элемент);
	КонецЕсли;
...
Процедура ЗарегистрироватьНаСервере()
	...
	ИнтеграцияС1СДокументооборот.
		ЗаполнитьРеквизитФормыИзXDTO(ЭтотОбъект, "ДатаРегистрации", Результат.document, "regDate");
	ИнтеграцияС1СДокументооборот.
		ЗаполнитьРеквизитФормыИзXDTO(ЭтотОбъект, "РегистрационныйНомер", Результат.document, "regNumber");
	ИнтеграцияС1СДокументооборот.
		ЗаполнитьРеквизитФормыИзXDTO(ЭтотОбъект, "Состояние", Результат.document, "status");
...
Процедура ПрочитатьОбъектВФорму(ОбъектXDTO)
	...
	Для Каждого Строка Из Стороны Цикл
		...
		legalPrivatePersonID = "";
		Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(СтрокаXDTO, "party")
				И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(СтрокаXDTO.party, "correspondent") Тогда
...
Процедура ЗаписатьОбъект(ПроверятьСвязи = Истина)
	...
	Если ЯвляетсяЗаявкойНаОплату
		И (ПолучательТип = "DMUser"
			Или ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаXDTO(ПолучательТип) = "DMUser") Тогда
		
		СвязьУжеЕсть = Ложь;
		АвансовыйОтчетXDTO = ИнтеграцияС1СДокументооборот.ПредопределенноеЗначениеДО(
...
Процедура ЗаполнитьДанныеЗаявкиНаОплату()
	...
	Если (ПолучательТип = "DMCorrespondent"
		Или ПолучательТип = ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаИС("DMCorrespondent"))
	...
	Если ЗначениеЗаполнено(ОрганизацияID) Тогда
		...
		НаименованиеПлательщикXDTO = ИнтеграцияС1СДокументооборот.ПредопределенноеЗначениеДО(
...
// Устанавливает доступность реквизитов документов зависимости от наличия подписи.
//
&НаСервере
...
Функция ДанныеНоменклатуры(НоменклатураТип, НоменклатураID)
	...
	ОбъектыXDTO = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(Прокси, НоменклатураТип, НоменклатураID);
	Если ОбъектыXDTO.objects.Количество() = 1 Тогда
		НоменклатураXDTO = ОбъектыXDTO.objects[0];
		ДанныеНоменклатуры.price = НоменклатураXDTO.price;
		Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НоменклатураXDTO, "measurementUnit")
				И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НоменклатураXDTO.measurementUnit, "objectID") Тогда
		...
		КонецЕсли;
		Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НоменклатураXDTO, "VATRate")
				И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(НоменклатураXDTO.VATRate, "objectID") Тогда
...
Процедура ЗаполнитьДополнительныеДанныеСторон(Строка, СторонаТип, legalPrivatePersonID)
	...
	Иначе
		СторонаТипXDTO = ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаXDTO(СторонаТип);
	КонецЕсли;
...
Процедура ПеренестиКонтрагентаИОрганизациюИзТЧСтороныВРеквизиты()
	...
	ИмяТипаКонтрагента = ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаИС("DMCorrespondent");
	...
	Если КоличествоСторон > 0
			И (Стороны[0].СторонаТип = "DMOrganization"
				Или ИнтеграцияС1СДокументооборотВызовСервера.ИмяТипаXDTO(Стороны[0].СторонаТип) = "DMOrganization") Тогда
...
Процедура ДополнитьСоответствиеРеквизитов(СоответствиеРеквизитов)
	...
	СоответствиеРеквизитов.Вставить("numberOfSheets", "СоставСтрока");
